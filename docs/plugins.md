...
The Memory activity is visible only when a workspace is selected and a compatible memory provider is detected for the active agent profile and project scope. A compatible provider with zero entries still shows the activity but omits its count badge. A positive count badge totals global and project-specific memory entries. PI WEBUI refreshes the badge immediately after a selected project or workspace change and then checks approximately every 30 seconds. This is polling; PI WEBUI does not promise instant or realtime updates.
