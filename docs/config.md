...
- The window is rolling 60 seconds, not aligned calendar minutes: an entry stops counting exactly 60 seconds after it was recorded, and a call is admitted only while the retained request count is below RPM and the retained token sum is below TPM.
...
- When a limit is exhausted, later calls to that model wait in first-in, first-out order. Waiting never spends a RPM unit. Cancelling a waiting call removes it before dispatch; an already-dispatched call keeps its unit and reports its usage.
