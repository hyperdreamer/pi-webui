...
- **Availability and compatibility:** Per-session policy uses `sessions.modelPolicy`; restoring and writing back the full starter preference additionally requires `sessions.modelPolicyDefaults` on both web and session daemon. Older peers retain version-one behavior.
