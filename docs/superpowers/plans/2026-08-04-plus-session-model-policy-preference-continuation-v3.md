# Plus-Created Session Model Policy Recovery Continuation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use the deterministic
> subagent-driven-development controller to implement this plan task-by-task.
> Steps use checkbox (- [ ]) syntax for tracking.

**Goal:** Re-establish independent credit for the negotiated browser parser fix, then complete the remaining plus-session model-policy recovery work through documentation and full verification.

**Architecture:** Start from the last credited Task 4 production implementation and reapply the one missing test contract as an independently reviewed task. Then execute the provenance, full-policy initialization, server-authoritative writeback, draft readiness, scheduling, Lit integration, and documentation boundaries in order, preserving the approved version-two negotiation and version-one compatibility contracts.
...
- This fresh recovery continuation starts from credited clean HEAD 92ba2fb40d32ec14aebd109601c357f3c9b38f21; preserve the uncredited cf1bca571e1e86805cd0da38cde2b58952064d8e on its safety branch only and do not treat it as evidence for this run.
- Task 1 independently re-establishes the T4-F-1 parser-wiring test contract and must modify only src/client/src/api/clients.test.ts; production code from the credited Task 4 implementation is unchanged.
- Tasks 2-8 map in order to the prior Recovery Tasks 5-11; prior terminal mismatch runs and uncredited child artifacts remain immutable evidence and are not silently rewritten.

## Task 1: Pin negotiated defaults parser wiring

**Implementer tier:** Standard

**Files:**

- Modify: src/client/src/api/clients.test.ts:409-490

**Interfaces:**

- Consumes: existing sessionsApi.sessionDefaultsV2(cwd: string, machineId?: string): Promise<SessionDefaultsV2Response> and parseSessionDefaultsV2Response(value: unknown): SessionDefaultsV2Response from the credited Task 4 implementation at 92ba2fb.
- Produces: a client regression test that fails if sessionsApi.sessionDefaultsV2() is wired to the legacy parseSessionDefaultsResponse() parser.
- Preserves: all production files, the legacy client URL assertion, and the exact local/selected-machine URL and selector-byte contract.

- [ ] **Step 1: Add resolved-value assertions to the negotiated client test**

In the existing "requests negotiated session defaults through local and selected-machine routes" test, retain the two sessionDefaultsV2() calls and every exact URL/search-parameter assertion. Capture both returned values with await expect(...).resolves.toEqual(v2Defaults), or equivalent explicit resolved-value assertions, so both local and selected-machine calls prove that the complete V2 response is returned. The assertions must include starterModelPolicyContractVersion: 2 and therefore must fail when the legacy parser drops that field.

Do not change src/client/src/api/clients.ts, src/client/src/api.ts, parser implementations, fixtures outside this test, or any unrelated test.

- [ ] **Step 2: Run the focused client test and inspect the changed scope**

Run: npm test -- --run src/client/src/api/clients.test.ts

Expected: PASS against the credited V2 client implementation. Inspect git diff -- src/client/src/api/clients.test.ts and confirm the only change is the two resolved-value assertions.

- [ ] **Step 3: Prove the regression test is falsifiable with a read-only mutant**

Create a temporary archive outside the active worktree from the fixed working tree, apply the test-only change to that archive, and replace only the parseSessionDefaultsV2Response argument in the new client method with parseSessionDefaultsResponse. Run the negotiated-defaults test in the archive:

~~~
npm test -- --run src/client/src/api/clients.test.ts
~~~

The mutant run must fail because the resolved object omits starterModelPolicyContractVersion. Record the failing assertion in the report, remove the archive, and verify the active worktree is unchanged apart from the intended test file. Do not modify or commit production code.

- [ ] **Step 4: Run static checks and commit the independent correction**
...
Inspect git status --porcelain and the exact diff. Commit only src/client/src/api/clients.test.ts:

~~~
git add src/client/src/api/clients.test.ts
git commit -m "test(model-policy): verify negotiated defaults parser"
~~~

Write the required report with STATUS: DONE, the two resolved-value assertions, the mutant failure evidence, verification results, and the commit SHA before returning.
...
- Consumes: existing `SessionStartOptions`, `StarterModelPolicyPreference`, and `SessionCreationSource`, plus `serializeSessionCreationSource()` / `inspectSessionCreationSource()` from Task 2.
...
- Consumes: source inspection from Task 2, newest `SessionModelPolicyInspection` from the existing policy module, and tagged full store writes from reviewed commit `90b7875`.
...
- Consumes: `sessionsApi.startPlusSession()` from Task 3, `sessionsApi.rememberCurrentModelPolicy()` from Task 4, complete starter policy, and projected `SessionInfo.creationSource` from Task 2.
...
- Consumes: the credited version-two defaults client boundary from the earlier negotiated-defaults work, starter evaluation from Task 5, plus start/confirmed callback and scoped writer from Task 6, and remember client from Task 4.
...
- Consumes: completed behavior and compatibility contract from Tasks 1-7 and the approved terminology already committed in CONTEXT.md.
