# Plus-Created Session Model Policy Memory Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use the deterministic
> subagent-driven-development controller to implement this plan task-by-task.
> Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Make each root created through SESSIONS `+` initialize from, and become the only eligible source of, the complete last server-confirmed model policy for the same machine/workspace.

**Architecture:** Extend the existing machine-local, normalized-workspace preference store to a strict version-two full-policy format while preserving version-one entries and clients. Persist a versioned `session-list-plus` marker in the session transcript, initialize eligible roots from a full policy before announcement, and expose a policy-free session command that rereads server truth before replacing the preference. Negotiate the new behavior through `sessions.modelPolicyStarterSelection`; older peers continue using the existing version-one starter path.

**Tech Stack:** TypeScript, Node.js filesystem APIs, Express, the Pi SDK session manager/custom entries, Lit, Vitest, Changesets, and PI WEBUI's existing selected-machine proxy/capability infrastructure.
...
## Task 1: Add complete starter-policy and creation contracts

**Implementer tier:** Advanced

**Files:**

- Modify: `src/shared/apiTypes.ts:60-180,560-600,1080-1180`
- Modify: `src/shared/capabilities.ts:1-75`
- Modify: `src/shared/capabilities.test.ts:1-215`
- Modify: `src/server/sessions/starterModelPolicyPreferenceStore.ts:1-35`
- Modify: `src/server/sessions/sessionDefaultsService.ts:1-35`
- Modify: `src/server/sessions/sessionDefaultsService.test.ts:1-20`
- Modify: `src/server/sessions/sessionDefaultsRoutes.ts:1-25`
- Modify: `src/server/sessiond.ts:1-80,620-700`
- Modify: `src/client/src/api/parsers.ts:1-80,650-760`
- Modify: `src/client/src/components/sessionModelPolicyDraft.ts:1-45`
- Modify: `src/client/src/components/PiWebUiApp.ts:1-55`
- Modify: `src/client/src/controllers/starterModelPolicyPreferenceWriter.ts:1-45`

**Interfaces:**

- Consumes: existing `ExactModelSelection`, `ModelTier`, `SessionModelPolicyUpdate`, `SessionInfo`, `SessionDefaultsResponse`, and capability helpers from `src/shared/apiTypes.ts` and `src/shared/capabilities.ts`.
- Produces: `LegacyStarterModelPolicyPreference`, `StarterModelPolicyPreference`, `StarterModelPolicyPreferenceResponse`, `SessionDefaultsV2Response`, `SessionCreationSource`, and `SessionStartOptions` with the exact shapes below.
- Produces: known capability key `PI_WEBUI_CAPABILITIES.sessionsModelPolicyStarterSelection = "sessions.modelPolicyStarterSelection"`, requiring both `web` and `sessiond`, but does not add it to either runtime advertisement array until Task 9.

- [ ] **Step 1: Write the failing shared-contract tests**

Add a capability test proving the key is known and effective only when both components advertise it. Add compile-time fixtures beside the relevant tests that exercise the two disjoint start request branches:
...
The capability test must assert that web-only and sessiond-only inputs omit the new capability, while a pair that both advertises it includes it.

- [ ] **Step 2: Run the focused test and confirm it fails**

Run: `npm test -- --run src/shared/capabilities.test.ts`

Expected: FAIL because `sessionsModelPolicyStarterSelection`, the full preference types, and the disjoint start contract do not exist.

- [ ] **Step 3: Add the shared types and mechanically separate legacy consumers**

Define these contracts in `src/shared/apiTypes.ts`:
...
Add `creationSource?: SessionCreationSource` to `SessionInfo`. Change the existing unversioned `SessionDefaultsResponse` and `SessionDefaultsUpdate` fields to `LegacyStarterModelPolicyPreference`, then update current store/service/route/client imports and annotations to that legacy name without changing runtime behavior. Add the capability key and its `EFFECTIVE_CAPABILITY_REQUIREMENTS` entry, leaving `WEB_RUNTIME_CAPABILITIES` and `SESSIOND_RUNTIME_CAPABILITIES` unchanged in this task.

- [ ] **Step 4: Run focused tests and static checks**

Run: `npm test -- --run src/shared/capabilities.test.ts src/client/src/components/sessionModelPolicyDraft.test.ts src/server/sessions/sessionDefaultsService.test.ts`

Run: `npm run typecheck`

Expected: PASS; current version-one behavior still compiles against the explicitly named legacy type, and the new capability is recognized without being advertised.

- [ ] **Step 5: Commit**

```bash
git add src/shared/apiTypes.ts src/shared/capabilities.ts src/shared/capabilities.test.ts src/server/sessions/starterModelPolicyPreferenceStore.ts src/server/sessions/sessionDefaultsService.ts src/server/sessions/sessionDefaultsService.test.ts src/server/sessions/sessionDefaultsRoutes.ts src/server/sessiond.ts src/client/src/api/parsers.ts src/client/src/components/sessionModelPolicyDraft.ts src/client/src/components/PiWebUiApp.ts src/client/src/controllers/starterModelPolicyPreferenceWriter.ts
git commit -m "feat(model-policy): add complete starter selection contracts"
```

## Task 2: Migrate the preference store to strict version two

**Implementer tier:** Capable

**Files:**

- Modify: `src/server/sessions/starterModelPolicyPreferenceStore.ts:1-320`
- Modify: `src/server/sessions/starterModelPolicyPreferenceStore.test.ts:1-230`
- Modify: `src/server/sessions/sessionDefaultsService.ts:25-190`
- Modify: `src/server/sessions/sessionDefaultsService.test.ts:20-135`

**Interfaces:**

- Consumes: `LegacyStarterModelPolicyPreference` and complete `StarterModelPolicyPreference` from Task 1.
- Produces: `StarterPreferenceInspection` and `StarterPreferenceWrite` exactly as specified below; `StarterModelPolicyPreferenceStore.inspect(cwd)` and `.replace(cwd, write)` clone values at the boundary.
- Preserves: `SessionDefaultsService.read(cwd): Promise<SessionDefaultsResponse>` down-projects a full stored value to version-one `{mode, tier?}`, and `update()` writes only `{kind: "legacy-v1", preference}` for the old update contract.

- [ ] **Step 1: Write failing migration and compatibility tests**

Extend `starterModelPolicyPreferenceStore.test.ts` with concrete full values and assertions for all four inspection variants:
...
await store.replace("/workspace/full", {
  kind: "full",
  preference: fullPreference,
});
expect(await store.inspect("/workspace/full")).toEqual({
  kind: "full",
  preference: fullPreference,
});
```

Write a version-one file with two workspaces, inspect one as `legacy-v1` without changing the file bytes, then replace a third workspace with `kind: "full"`. Assert the resulting JSON is version 2, the target has exactly `kind` plus `policy`, both untouched entries have exactly `kind` plus `preference`, and all original workspace keys remain. Add rejection cases for unknown fields, blank Exact fields, Tiered without tier, non-canonical tiers, malformed version-one entries, malformed version-two entries, and unsupported file versions.

Update `sessionDefaultsService.test.ts` so an old update expects:
...
Also assert that an unversioned read of a full inspection returns only `{mode, tier}` and never an `exact` key.

- [ ] **Step 2: Run the store and service tests and confirm they fail**

Run: `npm test -- --run src/server/sessions/starterModelPolicyPreferenceStore.test.ts src/server/sessions/sessionDefaultsService.test.ts`

Expected: FAIL because the store still understands only version 1 and accepts an untagged replacement.

- [ ] **Step 3: Implement the strict tagged store and incremental migration**

Export these types and preserve the existing serialized operation queue and atomic-private-file behavior:
...
Parse version-one files into an internal map of tagged legacy entries without writing during `inspect()`. Parse version-two entries with exact key sets:
...
On `replace()`, run read-parse-clone-replace-serialize-write as one queued operation. A valid version-one file becomes version 2 only when a write occurs; all untouched workspaces become explicit `legacy-v1` entries. Reject malformed input before opening a temporary replacement, retain mode `0o600`, clean failed temporary files, and continue the queue after a rejected save.

Adapt `SessionDefaultsService` to the tagged interface. Its unversioned read maps `full` and `legacy-v1` to a cloned legacy projection, maps `absent` to no preference, and maps `invalid` to `starterModelPolicyPreferenceError`. Its old update constructs only a tagged legacy write.
...
Expected: PASS, including serialized concurrent writes, atomic failure cleanup, private permissions, no eager migration, and strict full/legacy parsing.

- [ ] **Step 5: Commit**

```bash
git add src/server/sessions/starterModelPolicyPreferenceStore.ts src/server/sessions/starterModelPolicyPreferenceStore.test.ts src/server/sessions/sessionDefaultsService.ts src/server/sessions/sessionDefaultsService.test.ts
git commit -m "feat(model-policy): migrate starter preferences to version two"
```

## Task 3: Negotiate version-two session defaults

**Implementer tier:** Advanced

**Files:**

- Modify: `src/server/sessions/sessionDefaultsService.ts:35-210`
- Modify: `src/server/sessions/sessionDefaultsService.test.ts:1-180`
- Modify: `src/server/sessions/sessionDefaultsRoutes.ts:1-150`
- Modify: `src/server/sessions/sessionDefaultsRoutes.test.ts:1-260`
- Modify: `src/server/sessiond/sessionProxyRoutes.test.ts:1-260`
- Modify: `src/client/src/api/parsers.ts:650-780`
- Modify: `src/client/src/api/parsers.test.ts:409-486`
- Modify: `src/client/src/api/clients.ts:150-250`
- Modify: `src/client/src/api/clients.test.ts:409-445`
- Modify: `src/client/src/api.ts:1-6`

**Interfaces:**

- Consumes: tagged store inspection from Task 2 and `SessionDefaultsV2Response` / `StarterModelPolicyPreferenceResponse` from Task 1.
- Produces: `SessionDefaultsService.readV2(cwd: string): Promise<SessionDefaultsV2Response>`.
- Produces: strict client `parseSessionDefaultsV2Response(value: unknown): SessionDefaultsV2Response` and `sessionsApi.sessionDefaultsV2(cwd: string, machineId?: string): Promise<SessionDefaultsV2Response>`.
- Preserves: unversioned `GET /sessions/defaults?cwd=...` and all updates remain version one.

- [ ] **Step 1: Write failing service, route, proxy, parser, and client tests**

Add service cases for: full preference returned byte-for-byte; legacy Exact hydrated from raw configured provider/model/thinking even when that tuple is unavailable; legacy Tiered hydrated when raw Exact is complete; and incomplete raw Exact returned as a legacy shape rather than fabricated full intent.

Add route assertions:

```ts
await request(app)
  .get("/sessions/defaults")
  .query({ cwd: "/workspace", starterModelPolicyContract: "2" })
  .expect(200)
  .expect(({ body }) => {
    expect(body.starterModelPolicyContractVersion).toBe(2);
  });

expect(service.readV2).toHaveBeenCalledWith("/workspace");
expect(service.read).not.toHaveBeenCalled();
```

Assert missing negotiation calls `read()`, unsupported/duplicate contract values return 400, and the selected-machine proxy retains both encoded query fields. Add parser rejection cases for a missing/wrong contract version, unknown response/preference/exact/model keys, blank Exact fields, and Tiered without canonical tier. Add a client assertion that the URL contains `starterModelPolicyContract=2` exactly once under local and selected-machine prefixes.

- [ ] **Step 2: Run the focused tests and confirm they fail**

Run: `npm test -- --run src/server/sessions/sessionDefaultsService.test.ts src/server/sessions/sessionDefaultsRoutes.test.ts src/server/sessiond/sessionProxyRoutes.test.ts src/client/src/api/parsers.test.ts src/client/src/api/clients.test.ts`

Expected: FAIL because no version-two read, route negotiation, strict parser, or browser client exists.

- [ ] **Step 3: Implement raw-intent hydration and explicit negotiation**

Implement `readV2()` by loading Pi's raw configured values before any availability/clamping projection and inspecting the tagged store. Use this rule:
...
function hydrateLegacyPreference(
  ...
  if (exact === undefined) return { ...preference };
  return {
...
Only form `exact` when provider, model ID, and thinking level are all non-blank; do not consult available models or thinking levels for hydration. Return full inspections unchanged and clone them. Return valid Pi defaults plus `starterModelPolicyPreferenceError` for invalid inspection or read failure.

In the GET route, accept `starterModelPolicyContract=2` as the sole version-two selector and reject every other provided value. Keep unversioned reads on `read()`. In the client, build the query with `URLSearchParams`, resolve it once through existing request helpers, and parse through a dedicated strict version-two parser. Distinguish full from legacy preferences only by own required `exact` presence.
...
Expected: PASS; unversioned strict clients still receive no nested `exact`, and negotiated clients receive only contract version 2.
...
git commit -m "feat(model-policy): negotiate full starter defaults"
```

## Task 4: Persist and project plus-session creation source
...
- Consumes: `SessionCreationSource = "session-list-plus"` and optional `SessionInfo.creationSource` from Task 1.
...
## Task 5: Initialize plus-created roots from a complete policy
...
- Consumes: `SessionStartOptions`, `StarterModelPolicyPreference`, `SessionCreationSource`, and source serializer/parser from Tasks 1 and 4.
...
## Task 6: Add server-authoritative confirmed-policy writeback
...
- Consumes: source inspection from Task 4, newest `SessionModelPolicyInspection` from the existing policy module, and tagged full store write from Task 2.
...
## Task 7: Model full starter drafts and active-branch readiness
...
- Consumes: legacy/version-two defaults, complete starter preference, model-tier catalog, and existing immutable draft selectors from Tasks 1 and 3.
...
## Task 8: Schedule writeback from confirmed client session outcomes
...
- Consumes: `sessionsApi.startPlusSession()` from Task 5, `sessionsApi.rememberCurrentModelPolicy()` from Task 6, complete starter policy, and projected `SessionInfo.creationSource`.
...
## Task 9: Integrate negotiated full-policy behavior in the Lit application
...
- Consumes: version-two defaults client from Task 3, starter evaluation from Task 7, plus start/confirmed callback and scoped writer from Task 8, and remember client from Task 6.
...
## Task 10: Document, changeset, and verify the complete feature
...
- Consumes: completed behavior and compatibility contract from Tasks 1-9 and the approved terminology already committed in `CONTEXT.md`.
