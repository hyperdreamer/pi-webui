...
| `src/client/src/components/PiWebUiApp.clearQueue.test.ts` | 3 | **none** |
...
- `src/client/src/components/PiWebUiApp.clearQueue.test.ts`
...
2. **Slice B (client component tests):** Migrate each window component test to the
   ...
   `SettingsDialog.*` files + `SettingsDialog.testSupport.ts`, `PiWebUiApp.clearQueue.test.ts`,
