...
Install or update the local-build PI WEBUI Docker runtime. The installer refreshes
...
Options:
  --install-dir DIR       Install directory (default: $XDG_DATA_HOME/pi-webui-docker
                          or ~/.local/share/pi-webui-docker)
...
Progressive host setup:
  ...
  PI_WEBUI_DOCKER_EXTRA_HOST_PATHS to a whitespace-separated list of additional
...
if [ -n "$default_data_home" ]; then
  default_install_dir=$default_data_home/pi-webui-docker
fi
install_dir_input=${PI_WEBUI_DOCKER_HOME:-$default_install_dir}
[ -n "$install_dir_input" ] || die "HOME, XDG_DATA_HOME, or PI_WEBUI_DOCKER_HOME must be set"
install_dir=$(absolute_dir "$install_dir_input") || die "could not create install directory"
env_file=$install_dir/.env

asset_ref=$(value_from_env_or_existing_or_default PI_WEBUI_DOCKER_REF main)
asset_base=${PI_WEBUI_DOCKER_ASSET_BASE:-https://raw.githubusercontent.com/hyperdreamer/pi-webui/$asset_ref/docker}
use_local_asset_dir=1
if [ "${PI_WEBUI_DOCKER_REFRESH_ASSETS:-0}" = 1 ] || [ "${PI_WEBUI_DOCKER_REF+x}" = x ] || [ "${PI_WEBUI_DOCKER_ASSET_BASE+x}" = x ]; then
  use_local_asset_dir=0
fi

if [ "${PI_WEBUI_DOCKER_ASSET_DIR+x}" = x ]; then
  asset_dir=$(absolute_existing_dir "$PI_WEBUI_DOCKER_ASSET_DIR") || die "asset directory does not exist: $PI_WEBUI_DOCKER_ASSET_DIR"
  asset_base=
...
if [ -n "$asset_dir" ]; then
  ...
  profile_helper_temp=${TMPDIR:-/tmp}/pi-webui-host-profile.$$
...
if ! pi_webui_docker_host_detect_profile; then
...
write_asset pi-webui-docker 0755
...
pi_webui_uid=$(value_from_env_or_default PI_WEBUI_UID "$(id -u)")
pi_webui_gid=$(value_from_env_or_default PI_WEBUI_GID "$(id -g)")
docker_gid=$(value_from_env_or_default DOCKER_GID "$(pi_webui_docker_host_detect_docker_gid)")
pi_webui_host_profile=$PI_WEBUI_DETECTED_DOCKER_HOST_PROFILE
hostexec_mode=$PI_WEBUI_DETECTED_HOSTEXEC_MODE

raw_data_dir=$(value_from_env_or_existing_or_default PI_WEBUI_DOCKER_DATA_DIR "$install_dir/data")
...
pi_webui_extra_zypper_packages=$(value_from_env_or_existing_or_default PI_WEBUI_EXTRA_ZYPPER_PACKAGES "")
pi_webui_image=$(value_from_env_or_existing_or_default PI_WEBUI_IMAGE pi-webui:local)
compose_project_name=$(value_from_env_or_existing_or_default COMPOSE_PROJECT_NAME pi-webui)
hostexec_image=$(value_from_env_or_existing_or_default HOSTEXEC_IMAGE alpine:3.22)
pi_webui_max_upload_bytes=$(value_from_env_or_existing_or_default PI_WEBUI_MAX_UPLOAD_BYTES 67108864)
pi_webui_extra_host_paths=$(value_from_env_or_existing_or_default PI_WEBUI_DOCKER_EXTRA_HOST_PATHS "")

require_non_empty PI_WEBUI_UID "$pi_webui_uid"
require_non_empty PI_WEBUI_GID "$pi_webui_gid"
require_non_empty DOCKER_GID "$docker_gid"
require_non_empty PI_WEBUI_DOCKER_HOST_PROFILE "$pi_webui_host_profile"
require_non_empty HOSTEXEC_MODE "$hostexec_mode"
require_non_empty PI_WEBUI_DOCKER_DATA_DIR "$data_dir"
require_non_empty PI_WEBUI_DOCKER_INSTALL_DIR "$install_dir"
require_non_empty PI_WEBUI_DOCKER_REF "$asset_ref"
require_non_empty PI_WEBUI_BIND_ADDR "$pi_webui_bind_addr"
require_non_empty PI_WEBUI_PORT "$pi_webui_port"
require_non_empty PI_WEBUI_VERSION "$pi_webui_version"
require_non_empty PI_WEBUI_OPENSUSE_IMAGE "$pi_webui_opensuse_image"
require_non_empty PI_WEBUI_NODEJS_MAJOR "$pi_webui_nodejs_major"
require_non_empty PI_WEBUI_NODEJS_REPO "$pi_webui_nodejs_repo"
require_non_empty PI_WEBUI_IMAGE "$pi_webui_image"
require_non_empty COMPOSE_PROJECT_NAME "$compose_project_name"
require_non_empty HOSTEXEC_IMAGE "$hostexec_image"
require_non_empty PI_WEBUI_MAX_UPLOAD_BYTES "$pi_webui_max_upload_bytes"

pi_webui_extra_zypper_packages_env=$(dotenv_quote "$pi_webui_extra_zypper_packages")
pi_webui_extra_host_paths_env=$(dotenv_quote "$pi_webui_extra_host_paths")
compose_override_file=$install_dir/compose.override.yml
if ! pi_webui_docker_host_write_compose_override "$compose_override_file" "$pi_webui_host_profile" "$pi_webui_extra_host_paths" "$install_dir"; then
...
temp_env=$env_file.$$
cat >"$temp_env" <<EOF
# Generated by the PI WEBUI Docker installer.
# Re-run install.sh to refresh Docker assets and update the local image.
# Persistent data lives in PI_WEBUI_DOCKER_DATA_DIR and is not deleted by updates.

# Host identity used for the runtime containers and image user account.
PI_WEBUI_UID=$pi_webui_uid
PI_WEBUI_GID=$pi_webui_gid
DOCKER_GID=$docker_gid

# Detected Docker host profile and host capability toggles.
PI_WEBUI_DOCKER_HOST_PROFILE=$pi_webui_host_profile
HOSTEXEC_MODE=$hostexec_mode
PI_WEBUI_DOCKER_EXTRA_HOST_PATHS=$pi_webui_extra_host_paths_env

# Persistent data, Docker control root, and localhost-only default exposure.
PI_WEBUI_DOCKER_DATA_DIR=$data_dir
PI_WEBUI_DOCKER_INSTALL_DIR=$install_dir
PI_WEBUI_DOCKER_REF=$asset_ref
PI_WEBUI_BIND_ADDR=$pi_webui_bind_addr
PI_WEBUI_PORT=$pi_webui_port

# npm package selection. Pi resolves from PI WEBUI's npm peer dependency.
PI_WEBUI_VERSION=$pi_webui_version

# openSUSE/Node.js image build inputs.
PI_WEBUI_OPENSUSE_IMAGE=$pi_webui_opensuse_image
PI_WEBUI_NODEJS_MAJOR=$pi_webui_nodejs_major
PI_WEBUI_NODEJS_REPO=$pi_webui_nodejs_repo
PI_WEBUI_EXTRA_ZYPPER_PACKAGES=$pi_webui_extra_zypper_packages_env

# Runtime image names, Compose project, and limits.
PI_WEBUI_IMAGE=$pi_webui_image
COMPOSE_PROJECT_NAME=$compose_project_name
HOSTEXEC_IMAGE=$hostexec_image
PI_WEBUI_MAX_UPLOAD_BYTES=$pi_webui_max_upload_bytes
EOF
...
log "Selected PI WEBUI Docker host profile: $pi_webui_host_profile"
case "$pi_webui_host_profile" in
  linux-native-docker)
...
log "Persistent PI WEBUI Docker data: $data_dir"
log "Custom image hooks: $custom_image_hooks_dir"

if [ "${PI_WEBUI_DOCKER_SKIP_COMPOSE:-0}" = 1 ]; then
  log "Skipping Docker build/recreate because PI_WEBUI_DOCKER_SKIP_COMPOSE=1"
  exit 0
fi
...
log ""
log "WARNING: updating recreates the PI WEBUI Docker session daemon."
...
log ""
log "Building $pi_webui_image with --pull --no-cache (CACHE_BUST=$cache_bust) ..."
(
  cd "$install_dir"
  CACHE_BUST=$cache_bust run_runtime_compose build --pull --no-cache
)

log "Recreating split PI WEBUI Docker services ..."
(
  cd "$install_dir"
...
log ""
log "PI WEBUI Docker runtime is ready: http://$pi_webui_bind_addr:$pi_webui_port"
log "Install directory: $install_dir"
log "To update later, run: $install_dir/pi-webui-docker update"
(
  cd "$install_dir"
