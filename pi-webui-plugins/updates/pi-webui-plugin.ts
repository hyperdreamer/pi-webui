...
import type { HtmlTemplateTag, PiWebUiComponentStatus, PiWebUiPlugin, PiWebUiStatusResponse, PluginRuntimeState, WorkspacePanelTerminal } from "@hyperdreamer/pi-webui/plugin-api";
...
function renderComponent(html: HtmlTemplateTag, component: PiWebUiComponentStatus): TemplateResult {
...
function updatesRuntimeHintFromModuleUrl(moduleUrl: string): UpdatesRuntimeHint {
  try {
    const dockerMode = new URL(moduleUrl).searchParams.get("piWebUiDockerMode");
...
function renderCommands(html: HtmlTemplateTag, terminal: WorkspacePanelTerminal | undefined, status: PiWebUiStatusResponse): TemplateResult | undefined {
...
function renderUpdatesPanel(html: HtmlTemplateTag, terminal: WorkspacePanelTerminal | undefined, state: PluginRuntimeState | undefined): TemplateResult {
  ...
  if (status === undefined) {
    return html`
      ...
      <section class="viewer"><p class="muted">Checking PI WEBUI update status…</p></section>
    `;
  }
  ...
  return html`
    ...
    <section class="viewer updates-status">
      <section>
        ${messages.length === 0 ? html`<p class="muted">No PI WEBUI update or restart messages.</p>` : messages.map((message) => html`
  ...
  activate: ({ html, svg }) => ({
    contributions: {
      actions: [
        {
          id: "check",
          title: "Check for PI WEBUI Updates",
          description: "Bypass cached release data and check the selected machine now",
          group: "Updates",
          enabled: (context) => context.checkForPiWebUiUpdates !== undefined,
          disabledReason: () => "Update checks require a newer PI WEBUI gateway",
          run: (context) => context.checkForPiWebUiUpdates?.(),
        },
      ],
      workspacePanels: [
        {
