...
- 956dd5c: Add optional per-model TPM and RPM rate limits. Set them per model in **Models → Model configuration → Rate limits**; PI WEBUI queues calls in FIFO order over a rolling 60-second window and charges actual reported terminal tokens. Installing this change requires one manual `pi-webui-sessiond.service` restart.
...
- Upgrade the supported Pi Coding Agent package set to the 0.85.x release series (`0.85.1`).
...
- 4bf9291: Automatically dismiss transient speech-input errors after five seconds.
