...
describe("deterministic SDD pressure evaluator", () => {
  it("isolates candidate evaluation from discovered skills and sessions", () => {
    ...
    expect(invocation.args).toEqual([
      ...
      "--skill", expect.stringContaining("optional-skills/deterministic-subagent-driven-development"),
      "--extension",
      expect.stringContaining("optional-skills/deterministic-subagent-driven-development/evals/fake-sdd-tools.mjs"),
      "--model", "RightCode-OpenAI/gpt-5.6-sol:max",
    ...
    expect(JSON.parse(invocation.env.SDD_EVAL_READ_ROOTS_JSON)).toEqual([
      expect.stringContaining("optional-skills/deterministic-subagent-driven-development/SKILL.md"),
      expect.stringContaining("optional-skills/deterministic-subagent-driven-development/references"),
      expect.stringContaining("optional-skills/deterministic-subagent-driven-development/prompts"),
  ...
  it("loads the explicitly named original skill for the original condition", () => {
    const args = parseEvaluatorArgs([
      ...
      "--repetitions", "1", "--model", "p/m:high", "--output", "/tmp/out",
      "--original-skill", "/explicit/original/deterministic-subagent-driven-development",
    ]);
    const piArgs = buildPiInvocation(args, 1).args;
    expect(piArgs[piArgs.indexOf("--skill") + 1]).toBe("/explicit/original/deterministic-subagent-driven-development");
  });
