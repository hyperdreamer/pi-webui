...
import { PI_WEBUI_DOCKER_USER_COMMANDS } from "./piWebUiDockerCommandPlan.js";

const repoRoot = resolve(dirname(fileURLToPath(import.meta.url)), "..", "..");
const dockerOneLine = "curl -fsSL https://raw.githubusercontent.com/hyperdreamer/pi-webui/main/docker/install.sh | sh";

describe("pi-webui-docker documentation", () => {
  ...
  it("keeps Docker setup documentation scoped to the Docker folder", async () => {
    ...
    for (const content of nonDockerDocs) {
      ...
      expect(content).not.toContain("pi-webui-docker");
  ...
  it("keeps the Docker command matrix aligned with the canonical user command surface", async () => {
    const [dockerReadme, dockerEntrypoint] = await Promise.all([
      ...
      readRepoFile("docker/pi-webui-docker"),
    ]);

    expect(readDockerCommandMatrix(dockerReadme)).toEqual([...PI_WEBUI_DOCKER_USER_COMMANDS]);
    expect(readEntrypointCommandCases(dockerEntrypoint)).toEqual(new Set(PI_WEBUI_DOCKER_USER_COMMANDS));

    expect(dockerReadme).toContain("`pi-webui-docker --dev status`");
    expect(dockerReadme).toContain("`./docker/pi-webui-docker --dev start`");
    expect(dockerReadme).not.toContain("pi-webui-docker-control");
