...
import type { EffectivePiWebUiAgentConfig } from "../config.js";
import { createActiveAgentProfileDescriptor } from "./activeAgentProfile.js";

const baseAgent: EffectivePiWebUiAgentConfig = {
  command: "acme-agent",
...
describe("active agent profile descriptor", () => {
  it("builds a stable revision from every effective profile field", () => {
    ...
    expect(createActiveAgentProfileDescriptor({ ...baseAgent, sessionDirEnvKeys: ["PI_WEBUI_AGENT_SESSION_DIR", "PI_CODING_AGENT_SESSION_DIR"] }).revision).not.toBe(first.revision);
  });
  ...
  it("takes an immutable snapshot for the session daemon profile epoch", () => {
    ...
    expect(profile.sessionDirEnvKeys).toEqual(["PI_WEBUI_AGENT_SESSION_DIR"]);
  ...
  it("rejects profile fields outside the host and explicit environment policy", () => {
    ...
    expect(() => createActiveAgentProfileDescriptor({ ...baseAgent, sessionDirEnvKeys: ["ARBITRARY_AGENT_SESSION_DIR"] })).toThrow("explicit PI WEBUI policy");
  });
