...
import { isHostAbsoluteAgentDir, isSafeAgentCommandForHost, PI_CODING_AGENT_SESSION_DIR_ENV, PI_WEBUI_AGENT_SESSION_DIR_ENV, type EffectivePiWebUiAgentConfig } from "../config.js";
...
export function createActiveAgentProfileDescriptor(agent: EffectivePiWebUiAgentConfig): ActiveAgentProfileDescriptor {
  ...
  if (!hasValidSessionDirEnvKeys(agent.sessionDirEnvKeys)) {
    throw new Error("Active agent profile session directory environment keys must use the explicit PI WEBUI policy");
  }
...
function hasValidSessionDirEnvKeys(keys: readonly string[]): boolean {
  return (keys.length === 1 || keys.length === 2)
    && keys[0] === PI_WEBUI_AGENT_SESSION_DIR_ENV
