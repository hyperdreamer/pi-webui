...
import { PI_WEBUI_CAPABILITIES, supportsPiWebUiCapability, type PiWebUiCapability } from "../../../../shared/capabilities";
...
export function createCoreActions(): PluginAction[] {
  return [
    ...
    },
    {
      ...
      description: "Check whether the selected PI WEBUI runtime is online",
      group: "Machine",
      run: (context) => context.refreshSelectedMachine(),
    },
    {
      id: "machine.open",
      title: "Open Selected Machine PI WEBUI",
      description: "Open the selected remote PI WEBUI directly in a new tab",
      group: "Machine",
    ...
    },
    {
      ...
      description: "Reload the PI WEBUI browser page",
      group: "General",
...
function reloadSessionDisabledReason(context: { state: AppState }): string | undefined {
  ...
  return missingCapabilityReason(context.state, PI_WEBUI_CAPABILITIES.sessionsReload, "reload sessions from disk");
}
...
function missingCapabilityReason(state: AppState, capability: PiWebUiCapability, action: string): string | undefined {
  const runtime = state.machineRuntimes[selectedMachineId(state)];
  if (runtime?.ok === true && supportsPiWebUiCapability(runtime, capability)) return undefined;
