...
describe("PluginRegistry", () => {
  ...
  it("offers archive only for persisted sessions and delete only for transient new sessions", () => {
    ...
    const persistedStateRuntime = { local: { machineId: "local", ok: true as const, checkedAt: "now", capabilities: [PI_WEBUI_CAPABILITIES.sessionsPersistedState] } };
  ...
  it("enables session disk reload only for a writable session on a capable, idle runtime", () => {
    const registry = new PluginRegistry();
    registry.register({ id: "core", plugin: corePlugin });
    const reloadRuntime = { local: { machineId: "local", ok: true as const, checkedAt: "now", capabilities: [PI_WEBUI_CAPABILITIES.sessionsReload, PI_WEBUI_CAPABILITIES.sessionsPersistedState] } };
    const legacyReloadRuntime = { local: { machineId: "local", ok: true as const, checkedAt: "now", capabilities: [PI_WEBUI_CAPABILITIES.sessionsReload] } };
  ...
  it("routes session reload through the runtime context", () => {
    const registry = new PluginRegistry();
    registry.register({ id: "core", plugin: corePlugin });
    const { context, calls } = createContext({ selectedSession: testSession({ persisted: true }), machineRuntimes: { local: { machineId: "local", ok: true, checkedAt: "now", capabilities: [PI_WEBUI_CAPABILITIES.sessionsReload] } } });
  ...
  it("collects built-in PI WEBUI themes from an in-app plugin", () => {
    const registry = new PluginRegistry();
...
function createWorkspacePanelContext(machineId: string, prompt: WorkspacePanelContext["prompt"] = { insertText: vi.fn(), getText: vi.fn(() => ""), getSelection: vi.fn(() => null) }): WorkspacePanelContext {
  ...
  return {
    ...
    workspaceUploadDefaultFolder: ".pi-webui/uploads",
