...
export function parseProjectUsageCountResponse(value: unknown): ProjectUsageCountResponse {
  const record = requirePlainRecord(value, "project usage count response");
  assertOnlyFields(record, ["sessionCount"], "project usage count response");
  return { sessionCount: requireNonNegativeSafeInteger(record, "sessionCount") };
}
