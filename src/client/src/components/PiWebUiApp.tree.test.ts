...
// escape hatch only to verify PiWebUiApp's navigator callback boundary.
...
type RenderSessionTreeNavigator = (this: PiWebUiApp, state: AppState) => TemplateResult | null;
...
describe("PiWebUiApp session tree wiring", () => {
...
function setAppTree(app: PiWebUiApp, treeSnapshot: SessionTreeSnapshot): AppState {
  ...
  if (!Reflect.set(app, "state", state)) throw new Error("Could not set PiWebUiApp tree state");
  return state;
}

function renderSessionTreeNavigator(app: PiWebUiApp, state: AppState): TemplateResult {
  const method: unknown = Reflect.get(app, "renderSessionTreeNavigator");
  if (!isRenderSessionTreeNavigator(method)) throw new Error("PiWebUiApp.renderSessionTreeNavigator was unavailable");
