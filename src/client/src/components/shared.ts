...
export const appStyles = css`
  ...
  @media (max-width: 1180px) {
    ...
    main.workspace-view .empty, main.workspace-view .session-start-screen { display: none; }
  ...
  @media (max-width: 760px) {
    ...
    main.navigation-view .empty, main.navigation-view .session-start-screen { display: none; }
  ...
  .session-start-screen { flex: 1 1 auto; min-height: 0; display: grid; place-items: center; overflow: auto; box-sizing: border-box; padding: clamp(28px, 8vh, 96px) clamp(16px, 5vw, 48px); background: radial-gradient(circle at 50% 0%, color-mix(in srgb, var(--pi-accent) 13%, transparent), transparent 46%); }
  .session-start-content { width: min(100%, 760px); display: grid; gap: 16px; }
  .session-start-eyebrow { margin: 0; color: var(--pi-muted); font-size: 12px; font-weight: 650; letter-spacing: .09em; text-transform: uppercase; }
  ...
  @media (max-width: 640px) {
    ...
    .session-start-content h1 { font-size: clamp(30px, 10vw, 42px); }
  }
...
export const listStyles = css`
  ...
  .section-add-button { box-sizing: border-box; flex: 0 0 auto; display: inline-grid; place-items: center; min-width: 30px; height: 30px; padding: 0 9px; }
...
export const chatStyles = css`
  ...
  .chat { height: 100%; min-height: 0; overflow: auto; overflow-anchor: none; padding: 26px calc(16px + var(--pi-minimap-right, 0px)) 64px 16px; box-sizing: border-box; }
  .scroll-marker { display: block; height: 0; overflow: hidden; pointer-events: none; }
  .activity-dock { position: absolute; left: 16px; right: calc(16px + var(--pi-minimap-right, 0px)); bottom: 12px; z-index: 20; display: flex; align-items: center; gap: 8px; min-width: 0; box-sizing: border-box; border: 1px solid var(--pi-border); border-radius: 999px; background: var(--pi-bg-overlay); color: var(--pi-muted); padding: 8px 12px; font-size: 13px; pointer-events: none; box-shadow: 0 8px 28px var(--pi-shadow); backdrop-filter: blur(6px); }
  .activity-dock.active { border-color: var(--pi-success-border); color: var(--pi-success); background: var(--pi-success-bg-overlay); }
  .activity-leading { flex: 1 1 auto; min-width: 0; display: inline-flex; align-items: center; gap: 8px; }
  .activity-text { flex: 0 1 auto; min-width: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
  .activity-generation-metrics, .activity-metrics { flex: 0 0 auto; display: inline-flex; align-items: center; font-size: 12px; line-height: 1; white-space: nowrap; }
  .activity-generation-metrics { gap: 6px; }
  .activity-metrics { gap: 12px; margin-left: auto; }
  .activity-metric { flex: 0 0 auto; }
  .activity-metric.generation-rate { border-radius: 4px; color: var(--pi-text-bright); padding: 2px 6px; }
  .activity-metric.generation-rate.very-fast { background: var(--pi-accent); }
  .activity-metric.generation-rate.fast { background: var(--pi-success); }
  .activity-metric.generation-rate.moderate { background: var(--pi-warning); }
  .activity-metric.generation-rate.slow { background: var(--pi-danger); }
  .warning-toggle { flex: 0 0 auto; display: inline-flex; align-items: center; gap: 4px; border: 0; background: transparent; color: inherit; padding: 0; font: inherit; line-height: 1; white-space: nowrap; cursor: pointer; pointer-events: auto; }
  ...
  .usage-toggle { flex: 0 0 auto; display: inline-flex; align-items: center; border: 0; background: transparent; color: inherit; padding: 0; font: inherit; cursor: pointer; pointer-events: auto; border-radius: 4px; }
  .usage-toggle:hover { color: var(--pi-text-bright); }
  .usage-toggle:focus-visible { outline: 1px solid currentColor; outline-offset: 2px; }
  .usage-toggle .activity-metrics { pointer-events: none; }
  .dot { width: 8px; height: 8px; border-radius: 50%; background: currentColor; opacity: .45; flex: 0 0 auto; }
  .activity-dock.active .dot { animation: pulse 1s ease-in-out infinite; opacity: 1; }
  @media (max-width: 520px) {
    .activity-dock { left: 8px; right: calc(8px + var(--pi-minimap-right, 0px)); bottom: 8px; gap: 6px; padding: 7px 10px; }
    .activity-generation-metrics { gap: 4px; font-size: 11px; }
    .activity-metrics { gap: 8px; font-size: 11px; }
  }
  @media (max-width: 760px) {
    /* Minimap is hidden; reset padding so there is no dead space. */
    .chat { padding-right: 16px; }
    .activity-dock { right: 16px; }
  }
  ...
  .event-group-expand { border: 1px solid var(--pi-border); border-radius: 6px; background: var(--pi-surface); color: var(--pi-text-secondary); padding: 4px 8px; font: 12px system-ui, sans-serif; cursor: pointer; }
  .event-group-expand:hover, .event-group-expand:focus { border-color: var(--pi-accent); color: var(--pi-text-bright); }
  ...
  .session-info-popover-close:hover,
  .session-info-popover-close:focus-visible {
    color: var(--pi-text-bright);
    border-color: var(--pi-accent);
  }
  .session-info-popover-close:focus-visible {
    outline: 1px solid var(--pi-border);
    outline-offset: 2px;
  }
  .session-info-popover-body {
    flex: 1 1 auto;
    min-height: 0;
    overflow-y: auto;
  ...
  .session-info-detail-row dt {
    color: var(--pi-text-dim);
    white-space: nowrap;
    font-weight: normal;
  }
  .session-info-detail-row dd {
    display: flex;
    align-items: center;
    gap: 6px;
    min-width: 0;
  ...
  .session-info-copy:focus-visible {
    color: var(--pi-accent);
    border-color: var(--pi-accent);
