...
import type { PiWebUiConfigResponse, PiWebUiConfigValues, PiWebUiPluginInfo } from "../../api";
...
describe("settings-plugins-panel layout", () => {
  it("orders load and save notices before the trusted-code warning and plugin content", () => {
    ...
    panel.error = "Failed to load PI WEBUI plugin settings from Lab Mac: PI WEBUI plugins: timed out.";
    ...
    expectTextOrder(rendered, [
      "PI WEBUI plugins",
      "Enable or disable discovered PI WEBUI browser plugins on ",
      "Lab Mac (remote machine)",
      "Failed to load PI WEBUI plugin settings from Lab Mac: PI WEBUI plugins: timed out.",
  ...
  it("does not show a false empty state when the plugin response is missing", () => {
    ...
    expect(rendered).toContain("PI WEBUI plugin list unavailable for Lab Mac (remote machine). Use Reload to try again.");
    expect(rendered).not.toContain("No PI WEBUI browser plugins discovered");
  ...
  it("shows the empty plugin state only after a plugin response has loaded", () => {
    ...
    expect(rendered).toContain("No PI WEBUI browser plugins discovered on Lab Mac (remote machine).");
    expect(rendered).not.toContain("PI WEBUI plugin list unavailable");
    expect(rendered).not.toContain("Trusted code warning");
  });
...
function renderPluginTemplate(panel: SettingsPluginsPanel, plugin: PiWebUiPluginInfo): TemplateResult {
...
function isPanelRenderPlugin(value: unknown): value is (this: SettingsPluginsPanel, plugin: PiWebUiPluginInfo) => TemplateResult {
...
function pluginInfo(id: string, enabled: boolean): PiWebUiPluginInfo {
  return {
    id,
