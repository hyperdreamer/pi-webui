...
import type { PiWebUiConfigResponse, PiWebUiConfigValues, PiWebUiShortcutConfig } from "../../api";
...
export class SettingsShortcutsPanel extends LitElement {
  ...
  private async saveShortcutPreference(actionId: string, shortcut: string | null | undefined): Promise<void> {
    const config: PiWebUiConfigValues = { ...(this.configResponse?.config ?? {}) };
  ...
  private previewShortcutConfig(): PiWebUiShortcutConfig | undefined {
    const shortcuts = { ...(this.configResponse?.config.shortcuts ?? {}) };
...
function shortcutPreference(actionId: string, shortcuts: PiWebUiShortcutConfig | undefined): string | null | undefined {
  if (shortcuts === undefined || !Object.hasOwn(shortcuts, actionId)) return undefined;
  return shortcuts[actionId];
}

function withoutShortcutPreference(shortcuts: PiWebUiShortcutConfig, actionId: string): PiWebUiShortcutConfig {
  return Object.fromEntries(Object.entries(shortcuts).filter(([shortcutActionId]) => shortcutActionId !== actionId));
}

function effectiveShortcut(action: AppAction, shortcuts: PiWebUiShortcutConfig | undefined): string | undefined {
  const configured = shortcutPreference(action.id, shortcuts);
  if (configured === null) return undefined;
  return configured ?? action.shortcut;
}

function shortcutState(action: AppAction, shortcuts: PiWebUiShortcutConfig | undefined): ShortcutState {
  const configured = shortcutPreference(action.id, shortcuts);
