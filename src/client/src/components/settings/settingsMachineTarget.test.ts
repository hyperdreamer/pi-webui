...
describe("selected-machine settings target helpers", () => {
  ...
  it("gates remote selected-machine settings on advertised runtime support", () => {
    ...
    const supportedRuntime: MachineRuntime = { machineId: "remote-a", ok: true, checkedAt: "now", capabilities: [PI_WEBUI_CAPABILITIES.selectedMachineSettings] };
    const unsupportedRuntime: MachineRuntime = { machineId: "remote-a", ok: true, checkedAt: "now", capabilities: [PI_WEBUI_CAPABILITIES.piPackagesManage] };
  ...
  it("gates remote agent profile edits on their granular capability", () => {
    ...
    expect(agentProfileSettingsSupport(target, {
      ok: true,
      capabilities: [PI_WEBUI_CAPABILITIES.agentProfileConfig],
    })).toEqual({ state: "supported" });

    const unsupported = agentProfileSettingsSupport(target, { ok: true, capabilities: [PI_WEBUI_CAPABILITIES.selectedMachineSettings] });
  ...
  it("turns older remote config route failures into selected-machine compatibility guidance", () => {
    ...
    expect(selectedMachineSettingsUnavailableMessage(target)).toBe("Selected-machine settings are not available on Lab Mac. Update and restart PI WEBUI on that machine, then try again.");
