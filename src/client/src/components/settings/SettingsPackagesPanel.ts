...
export class SettingsPackagesPanel extends LitElement {
  ...
  private panelNotices(targetLabel: string, showTrustedCodeWarning: boolean): readonly SettingsNotice[] {
    ...
    if (showTrustedCodeWarning) {
      notices.push({
        type: "security",
        content: html`<strong>Trusted code warning:</strong> Pi packages and PI WEBUI plugins can run with your user permissions. Install packages and enable plugins only from sources you trust.`,
      });
    }
    return notices;
  }
  ...
  private renderInstallForm(targetLabel: string): TemplateResult {
    return html`
      <form class="install-card" @submit=${(event: Event) => { void this.installPackage(event); }}>
        ...
        <small>Installs run on ${targetLabel} and use Pi's default package location, equivalent to <code>pi install &lt;source&gt;</code>. PI WEBUI does not ask you to choose an install location.</small>
      </form>
    `;
  }
...
function packagesDescription(targetLabel: string): TemplateResult {
  return html`Managing Pi packages on <strong>${targetLabel}</strong>. Install, remove, and update packages managed by Pi on the selected machine. Pi packages can provide extensions, skills, prompt templates, themes, context/system prompt files, and PI WEBUI browser plugins.`;
}
