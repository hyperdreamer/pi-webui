...
export class SettingsSessiondPanel extends LitElement {
        ...
        ${config === undefined ? this.renderUnavailableConfigState() : html`
          ...
          <form class="profile-form" aria-label="Pi-compatible agent profile" @submit=${(event: Event) => { void this.saveAgentProfile(event); }}>
            ...
            <label class="field">
              ...
              >
              <small>Set the Pi-compatible companion CLI used for doctor and update checks. The embedded session runtime remains PI WEBUI's bundled Pi SDK.</small>
            </label>
            <label class="field">
              <span class="field-heading">
              ...
              >
              <small>Choose the Pi-compatible auth, models, settings, and sessions PI WEBUI reads. An alternate command and its required state directory are saved together.</small>
            </label>
  ...
  private panelNotices(config: PiWebUiConfigResponse | undefined): readonly SettingsNotice[] {
...
export function sessiondPanelNotices(
  ...
  if (activation === "restart-required") {
    notices.push({
      ...
      content: html`The desired profile differs from the active session-daemon profile. Run <code>pi-webui restart</code> on that machine (or restart its session daemon service) to apply the command and state directory together.`,
    });
    ...
    notices.push({
      ...
      content: "PI WEBUI cannot compare the desired profile with the running session daemon. Reload after the daemon is available.",
    });
  }
  return notices;
}
