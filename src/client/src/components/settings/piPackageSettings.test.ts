...
const runtimeWithPackageManagement: MachineRuntime = { machineId: "remote-a", ok: true, checkedAt: "now", capabilities: [PI_WEBUI_CAPABILITIES.piPackagesManage] };
const runtimeWithoutPackageManagement: MachineRuntime = { machineId: "remote-a", ok: true, checkedAt: "now", capabilities: [PI_WEBUI_CAPABILITIES.sessionsReload] };
...
describe("Pi package settings helpers", () => {
  ...
  it("formats package metadata with Pi package terminology", () => {
    ...
    expect(piPackageFilteredLabel(userPackage)).toBe("Available in this PI WEBUI process");
  ...
  it("describes the browser and session reload follow-up without requiring sessiond restarts", () => {
    ...
    expect(message).toContain("Type /reload in each idle PI WEBUI session");
    expect(message).toContain("extensions, skills, prompt templates, themes, and context/system prompt files");
    expect(message).toContain("Reload the browser page separately for PI WEBUI browser plugin changes");
  ...
  it("scopes remote package mutation follow-up copy to the selected machine", () => {
    ...
    expect(message).toContain("each idle PI WEBUI session on Lab Mac");
    expect(message).toContain("PI WEBUI browser plugin changes served by Lab Mac");
  });
