...
type SaveHandler = (config: PiWebUiConfigValues) => void | Promise<void>;
type TemplateEventHandler<E extends Event> = (event: E) => void;

function panelWithShortcuts(config: PiWebUiConfigValues, onSave: SaveHandler): SettingsShortcutsPanel {
  const panel = new SettingsShortcutsPanel();
