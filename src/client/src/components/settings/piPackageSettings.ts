...
export function piPackageManagementSupport(target: PiPackageTargetContext, runtime: Pick<MachineRuntime, "ok" | "capabilities"> | undefined): PiPackageManagementSupport {
  ...
  if (supportsPiWebUiCapability(runtime, PI_WEBUI_CAPABILITIES.piPackagesManage)) return { state: "supported" };
...
export function piPackageFilteredLabel(packageInfo: Pick<PiPackageInfo, "filtered">): string {
  return packageInfo.filtered ? "Filtered by current Pi package settings" : "Available in this PI WEBUI process";
}
...
export function piPackageUpdateDisabledReason(packageInfo: Pick<PiPackageInfo, "scope">): string | undefined {
  ...
  return "Project-scope Pi packages are listed for visibility, but PI WEBUI only updates user-scope Pi packages safely from this view.";
}
...
export function piPackageMutationFollowUpMessage(action: PiPackageMutationAction, target = piPackageTargetContext(undefined)): string {
  ...
  const sessionScope = target.kind === "local" ? "each idle PI WEBUI session" : `each idle PI WEBUI session on ${target.name}`;
  const pluginScope = target.kind === "local" ? "PI WEBUI browser plugin changes" : `PI WEBUI browser plugin changes served by ${target.name}`;
