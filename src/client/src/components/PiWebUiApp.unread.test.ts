...
// verifies only PiWebUiApp's unread-state property wiring into navigation.
...
describe("PiWebUiApp session unread wiring", () => {
  ...
  it("fetches initial state only after the selected runtime advertises unread support", async () => {
    ...
    setState(app, {
      machineRuntimes: {
        local: { machineId: "local", ok: true, checkedAt: "now", capabilities: [PI_WEBUI_CAPABILITIES.sessionsUnread] },
      },
    });
  ...
  it("renegotiates cached support before refreshing after a runtime rollback", async () => {
    ...
    setAppState(app, {
      ...
      selectedMachine: localMachine,
      machineRuntimes: {
        local: { machineId: "local", ok: true, checkedAt: "before", capabilities: [PI_WEBUI_CAPABILITIES.sessionsUnread] },
      },
    });
    const machines: unknown = Reflect.get(app, "machines");
    if (typeof machines !== "object" || machines === null) throw new Error("PiWebUiApp machine controller is unavailable");
  ...
  it("does not treat the legacy browser-local key as unread authority", () => {
    const app = createApp({ "pi-webui-session-unread-v1": JSON.stringify([["local", "background"]]) });
    const foreground = session("foreground");
    const background = session("background");
    setAppState(app, { ...initialAppState(), sessions: [foreground, background], selectedSession: foreground, mainView: "chat" });
...
type SetAppState = (this: PiWebUiApp, patch: Partial<AppState>) => void;
type HandleRealtimeEvent = (this: PiWebUiApp, machineId: string, event: BrowserRealtimeEvent) => void;
type MobileMainTabs = (this: PiWebUiApp) => AppMobileMainTab[];
type UpdatedHook = (this: PiWebUiApp) => void;
type DisconnectedHook = (this: PiWebUiApp) => void;
type RenegotiateUnreadMachine = (this: PiWebUiApp, machineId: string) => Promise<void>;
type RefreshUnread = (machineId: string) => Promise<void>;

function createApp(storedValues: Record<string, string> = {}, mobileNavigation = false): PiWebUiApp {
...
function setState(app: PiWebUiApp, patch: Partial<AppState>): void {
  const method: unknown = Reflect.get(app, "setState");
  if (!isSetAppState(method)) throw new Error("PiWebUiApp.setState is not callable");
  method.call(app, patch);
}

function handleRealtimeEvent(app: PiWebUiApp, event: BrowserRealtimeEvent): void {
  const method: unknown = Reflect.get(app, "handleRealtimeEvent");
  if (!isHandleRealtimeEvent(method)) throw new Error("PiWebUiApp.handleRealtimeEvent is not callable");
  method.call(app, "local", event);
}

function enableUnread(app: PiWebUiApp): void {
  if (!Reflect.set(app, "unreadConnected", true)) throw new Error("Could not connect PiWebUiApp unread state");
  const controller: unknown = Reflect.get(app, "sessionUnread");
  if (typeof controller !== "object" || controller === null) throw new Error("PiWebUiApp unread controller is unavailable");
  const setCapability: unknown = Reflect.get(controller, "setCapability");
  if (typeof setCapability !== "function") throw new Error("PiWebUiApp unread capability setter is unavailable");
  Reflect.apply(setCapability, controller, ["local", "supported"]);
}

function exposeSelectedChat(app: PiWebUiApp): void {
...
function invokeUpdated(app: PiWebUiApp): void {
  const method: unknown = Reflect.get(app, "updated");
  if (!isUpdatedHook(method)) throw new Error("PiWebUiApp.updated is not callable");
  method.call(app);
}
...
async function renegotiateUnreadMachine(app: PiWebUiApp, machineId: string): Promise<void> {
  const method: unknown = Reflect.get(app, "renegotiateUnreadMachine");
  if (!isRenegotiateUnreadMachine(method)) throw new Error("PiWebUiApp unread renegotiation is not callable");
  await method.call(app, machineId);
}

function refreshUnread(app: PiWebUiApp, machineId: string): Promise<void> {
  const controller: unknown = Reflect.get(app, "sessionUnread");
  if (typeof controller !== "object" || controller === null) throw new Error("PiWebUiApp unread controller is unavailable");
  const refresh: unknown = Reflect.get(controller, "refresh");
  if (!isRefreshUnread(refresh)) throw new Error("PiWebUiApp unread refresh is not callable");
  return refresh.call(controller, machineId);
}

function mobileNavigationTab(app: PiWebUiApp): AppMobileMainTab {
  const method: unknown = Reflect.get(app, "mobileMainTabs");
  if (!isMobileMainTabs(method)) throw new Error("PiWebUiApp.mobileMainTabs is not callable");
...
function navigationUnreadSessionIds(app: PiWebUiApp): ReadonlySet<string> {
