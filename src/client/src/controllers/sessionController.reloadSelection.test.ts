...
describe("SessionController reload and selection", () => {
  it("reloads the selected session from disk, discards the cached transcript, and re-fetches history", async () => {
    ...
    let state: AppState = {
      ...
      machineRuntimes: { local: { machineId: "local", ok: true, checkedAt: "now", capabilities: [PI_WEBUI_CAPABILITIES.sessionsReload] } },
    };
  ...
  it("does not reload sessions from disk without a persisted server signal when persistence is authoritative", async () => {
    ...
    let state: AppState = {
      ...
      sessions: [oldSession],
      machineRuntimes: { local: { machineId: "local", ok: true, checkedAt: "now", capabilities: [PI_WEBUI_CAPABILITIES.sessionsReload, PI_WEBUI_CAPABILITIES.sessionsPersistedState] } },
    };
    const api: typeof defaultApi = {
      ...defaultApi,
      reloadSession: (session) => {
