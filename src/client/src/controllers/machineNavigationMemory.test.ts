...
describe("SessionStorageMachineNavigationMemory", () => {
  ...
  it("ignores malformed stored snapshots", () => {
    const storage = memoryStorage({
      "pi-webui:machine-navigation:v1": JSON.stringify({ version: 1, entries: [["local", { machineId: "local", tool: "bad", surface: { selectedFilePath: "README.md" } }], ["remote", { projectId: "missing-machine", surface: {} }]] }),
    });
