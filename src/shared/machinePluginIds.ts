import { isPiWebUiPluginId } from "./pluginIds.js";
...
export function machineScopedPluginId(machineId: string, pluginId: string): string {
  ...
  if (!isPiWebUiPluginId(pluginId)) throw new Error(`Invalid PI WEBUI plugin id: ${pluginId}`);
...
export function parseMachineScopedPluginId(pluginId: string): MachineScopedPluginIdParts | undefined {
  ...
  if (!isHexString(encodedMachineId) || !isPiWebUiPluginId(sourcePluginId)) return undefined;
