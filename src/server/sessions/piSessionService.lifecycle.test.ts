...
describe("PiSessionService lifecycle, listing, and reload", () => {
  ...
  it("publishes provider-reported generation metrics only while an assistant response is active", async () => {
    ...
    const hub = new CapturingSessionEventHub();
    const fake = fakeRuntime("generation-session", { isStreaming: true });
    const service = new PiSessionService(hub, {
      agentDir: TEST_AGENT_DIR,
      modelRuntime: testModelRuntime,
      createAgentRuntime: runtimeCreator(fake.runtime),
      sessionManager: sessionGateway([sessionRecord("generation-session")]),
      heartbeatIntervalMs: 60_000,
      now: () => now,
    });
    const assistantMessage = (output: number) => ({
      role: "assistant",
    ...
    try {
      await service.status(sessionRef("generation-session"));
      fake.emit({ type: "message_start", message: assistantMessage(0) });
      now = new Date("2026-07-22T00:00:07.600Z");
      fake.emit({ type: "message_update", message: assistantMessage(32), assistantMessageEvent: { type: "text_delta", delta: "partial" } });

      const reported = await service.status(sessionRef("generation-session"));
      expect(reported.generation?.outputTokens).toBe(32);
      expect(reported.generation?.tokensPerSecond).toBeCloseTo(32 / 7.6);
      expect(reported.generation?.estimated).toBeUndefined();

      fake.emit({ type: "message_end", message: assistantMessage(32) });
      const idle = await service.status(sessionRef("generation-session"));
      expect(idle.generation).toBeUndefined();

      fake.emit({ type: "message_start", message: { ...assistantMessage(0), content: [] } });
      const withoutProviderUsage = await service.status(sessionRef("generation-session"));
      expect(withoutProviderUsage.generation).toBeUndefined();

      fake.emit({ type: "message_update", message: assistantMessage(0), assistantMessageEvent: { type: "text_delta", delta: "brief" } });
      now = new Date("2026-07-22T00:00:07.800Z");
      fake.emit({ type: "message_update", message: assistantMessage(8), assistantMessageEvent: { type: "text_delta", delta: "brief" } });
      const briefGeneration = await service.status(sessionRef("generation-session"));
      expect(briefGeneration).toMatchObject({ generation: { outputTokens: 8 } });
      expect(briefGeneration.generation?.tokensPerSecond).toBeUndefined();
    } finally {
      await service.dispose();
    }
  });

  it("estimates generation metrics from streamed assistant content when provider usage is unavailable", async () => {
    let now = new Date("2026-07-22T00:00:00.000Z");
    const hub = new CapturingSessionEventHub();
    const fake = fakeRuntime("estimated-generation-session", { isStreaming: true });
    const service = new PiSessionService(hub, {
      agentDir: TEST_AGENT_DIR,
      modelRuntime: testModelRuntime,
      createAgentRuntime: runtimeCreator(fake.runtime),
      sessionManager: sessionGateway([sessionRecord("estimated-generation-session")]),
      heartbeatIntervalMs: 60_000,
      now: () => now,
    });
    ...
    try {
      await service.status(sessionRef("estimated-generation-session"));
      fake.emit({ type: "message_start", message: assistantMessage([]) });
      fake.emit({ type: "message_update", message: assistantMessage(initialContent), assistantMessageEvent: { type: "text_delta", delta: "abcd" } });

      expect((await service.status(sessionRef("estimated-generation-session"))).generation).toEqual({ outputTokens: 3, estimated: true });

      now = new Date("2026-07-22T00:00:01.000Z");
      fake.emit({ type: "message_update", message: assistantMessage([
        { type: "text", text: "abcdefghijkl" },
        { type: "thinking", thinking: "efgh" },
        { type: "toolCall", id: "tool-1", name: "read", arguments: {} },
      ]), assistantMessageEvent: { type: "text_delta", delta: "efghijkl" } });
      expect((await service.status(sessionRef("estimated-generation-session"))).generation).toEqual({ outputTokens: 5, tokensPerSecond: 5, estimated: true });
    } finally {
      await service.dispose();
    }
  });

  it("clears generation metrics if an agent ends without an assistant message_end event", async () => {
    let now = new Date("2026-07-22T00:00:00.000Z");
    const hub = new CapturingSessionEventHub();
    const fake = fakeRuntime("generation-abort-session", { isStreaming: true });
    const service = new PiSessionService(hub, {
      agentDir: TEST_AGENT_DIR,
      modelRuntime: testModelRuntime,
      createAgentRuntime: runtimeCreator(fake.runtime),
      sessionManager: sessionGateway([sessionRecord("generation-abort-session")]),
      heartbeatIntervalMs: 60_000,
      now: () => now,
    });
    const assistantMessage = (output: number) => ({
    ...
    try {
      await service.status(sessionRef("generation-abort-session"));
      fake.emit({ type: "message_start", message: assistantMessage(0) });
      now = new Date("2026-07-22T00:00:01.000Z");
      fake.emit({ type: "message_update", message: assistantMessage(8), assistantMessageEvent: { type: "text_delta", delta: "partial" } });
      expect((await service.status(sessionRef("generation-abort-session"))).generation?.outputTokens).toBe(8);

      fake.emit({ type: "agent_end" });
      expect((await service.status(sessionRef("generation-abort-session"))).generation).toBeUndefined();
    } finally {
      await service.dispose();
    }
  });
  ...
  it("reports persistence from actual session-file existence for fresh active sessions", async () => {
    const dir = await mkdtemp(join(tmpdir(), "pi-webui-persisted-"));
  ...
  it("stores every extension notification without touching Pi session history", async () => {
    ...
    const canonicalCwd = resolve(tmpdir(), "pi-webui-notification-workspace");
  ...
  it("runs /reload by refreshing the active runtime resources in place", async () => {
    ...
    await expect(service.runCommand(sessionRef("runtime-reload-session"), "/reload")).resolves.toEqual({
      type: "done",
      message: "Session runtime resources reloaded. Extensions, skills, prompt templates, themes, and context/system prompt files are refreshed for this session. Reload the browser page separately for PI WEBUI browser plugin changes.",
    });
