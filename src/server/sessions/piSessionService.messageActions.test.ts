...
describe("PiSessionService per-message actions", () => {
  ...
  it("omits Pi's transcript system updates while keeping fork bookkeeping", async () => {
    const entries = [
      messageEntry("user-1", null, "user", "Start here"),
      messageEntry("assistant-1", "user-1", "assistant", "First answer"),
      {
        ...
        message: { role: "system", content: "", toolsAdded: [{ name: "read" }], timestamp: 1 },
      },
      messageEntry("user-2", "system-1", "user", "Revise this"),
    ];
    const manager = fakeSessionManager("/workspace", {
      getSessionId: () => SESSION_ID,
      getBranch: () => entries,
