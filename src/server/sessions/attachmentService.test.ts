...
beforeEach(async () => {
  ...
  workspace = await mkdtemp(join(tmpdir(), "pi-webui-attachments-"));
  externalDirectories = [];
});
...
describe("saveAttachmentsToWorkspace", () => {
  it("writes attachments into the default folder and returns relative paths", async () => {
    ...
    const folderEntries = await readdir(join(workspace, ".pi-webui", "attachments"));
  ...
  it("rejects attachment folders that resolve outside the workspace", async () => {
    const outside = await mkdtemp(join(tmpdir(), "pi-webui-attachments-outside-"));
    externalDirectories.push(outside);
    await mkdir(join(workspace, ".pi-webui"));
    await symlink(outside, join(workspace, ".pi-webui", "attachments"), "dir");
