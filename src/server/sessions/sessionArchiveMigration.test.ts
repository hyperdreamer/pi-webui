...
describe("legacy session archive migration preflight", () => {
  ...
  it("skips when PI_WEBUI_DATA_DIR is not explicitly configured", async () => {
    const fixture = await createLegacyArchiveFixture();
  ...
  it("skips when the configured data root resolves to the legacy root", async () => {
    ...
    await expect(migrateLegacySessionArchive({
      ...fixture.options,
      env: { PI_WEBUI_DATA_DIR: fixture.legacyRoot },
...
async function createLegacyArchiveFixture(
  ...
  const root = await mkdtemp(join(tmpdir(), "pi-webui-archive-migration-"));
  tempRoots.push(root);
  const homeDir = join(root, "home");
  const legacyRoot = join(homeDir, ".pi-webui");
  ...
  return {
    ...
    document,
    options: {
      env: { PI_WEBUI_DATA_DIR: destinationRoot },
      cwd: root,
      homeDir,
