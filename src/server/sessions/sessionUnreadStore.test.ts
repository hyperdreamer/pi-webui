...
describe("FileSessionUnreadPersistence", () => {
  ...
  it("uses PI_WEBUI_DATA_DIR and atomically reloads a private state file", async () => {
    const root = await temporaryRoot();
    expect(defaultSessionUnreadFilePath({ PI_WEBUI_DATA_DIR: "state" }, root)).toBe(join(root, "state", "session-unread.json"));
...
async function temporaryRoot(): Promise<string> {
  const root = await mkdtemp(join(tmpdir(), "pi-webui-unread-"));
  roots.push(root);
  return root;
}
