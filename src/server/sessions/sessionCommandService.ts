...
export class SessionCommandService<TSession extends CommandSession = CommandSession> {
  ...
  private async reload(session: TSession): Promise<ClientCommandResult> {
    ...
    return { type: "done", message: "Session runtime resources reloaded. Extensions, skills, prompt templates, themes, and context/system prompt files are refreshed for this session. Reload the browser page separately for PI WEBUI browser plugin changes." };
  }
