...
export function registerMachinePluginProxyRoutes(app: FastifyInstance, machines: MachinePluginProxyMachines = new MachineService()): void {
  app.get<{ Params: { machineId: string } }>("/api/machines/:machineId/pi-webui-plugins/manifest.json", async (request, reply) => {
    ...
    try {
      const response = await client.requestJson("GET", "/pi-webui-plugins/manifest.json", undefined, { timeoutMs: MACHINE_PLUGIN_MANIFEST_TIMEOUT_MS });
...
export async function proxyMachinePluginAsset(machines: MachinePluginProxyMachines, scopedPluginId: string, assetPath: string, requestUrl: string, reply: FastifyReply): Promise<boolean> {
  ...
  if (requestPath === undefined) {
    await reply.code(400).send({ error: "Invalid remote PI WEBUI plugin asset path" });
    return true;
  }
...
function rewriteRemotePluginManifest(machineId: string, manifest: RemotePluginManifest): RemotePluginManifest {
  return {
    plugins: manifest.plugins.flatMap((plugin) => {
      ...
      return [{
        ...plugin,
        module: `../../../../pi-webui-plugins/${encodeURIComponent(machineScopedPluginId(machineId, plugin.id))}/${modulePath.path}${modulePath.query}`,
      }];
    }),
  };
}
...
function remotePluginModulePath(pluginId: string, module: string): { path: string; query: string } | undefined {
  ...
  const prefix = `/pi-webui-plugins/${encodeURIComponent(pluginId)}/`;
  const pluginRootUrl = new URL(prefix, "http://pi-webui.local");
  const manifestUrl = new URL("/pi-webui-plugins/manifest.json", pluginRootUrl);
  try {
...
function remotePluginAssetRequestPath(remotePlugin: MachineScopedPluginIdParts, assetPath: string, requestUrl: string): string | undefined {
  ...
  return `/pi-webui-plugins/${encodeURIComponent(remotePlugin.pluginId)}/${path}${query}`;
}
...
function parseRemoteManifest(value: unknown): RemotePluginManifest {
  if (!isRecord(value) || !Array.isArray(value["plugins"])) throw new Error("Invalid remote PI WEBUI plugin manifest");
  return {
    plugins: value["plugins"].map((entry) => {
      if (!isRecord(entry) || typeof entry["id"] !== "string" || !isPiWebUiPluginId(entry["id"]) || typeof entry["module"] !== "string" || entry["module"] === "") {
        throw new Error("Invalid remote PI WEBUI plugin manifest entry");
      }
      return {
        id: entry["id"],
        module: entry["module"],
...
function parseRemoteMachineSpecific(value: unknown): { machineSpecific?: boolean } {
  ...
  if (typeof value !== "boolean") throw new Error("Invalid remote PI WEBUI plugin manifest entry");
