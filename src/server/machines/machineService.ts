import type { Machine, MachineHealth, MachineRuntime, PiWebUiComponentStatus, PiWebUiRuntimeComponent, PiWebUiRuntimeResponse, PiWebUiStatusResponse } from "../../shared/apiTypes.js";
import { parsePiWebUiRuntimeResponse } from "../../shared/piWebUiStatusParsing.js";
import { getPiWebUiRuntime } from "../piWebUiStatus.js";
...
export interface MachineServiceDependencies {
  localRuntime?: () => Promise<PiWebUiRuntimeResponse>;
...
export class MachineService {
  ...
  private async localHealth(): Promise<MachineHealth> {
    ...
    try {
      const runtime = await (this.deps.localRuntime ?? getPiWebUiRuntime)();
      return {
        machineId: "local",
        ok: true,
        checkedAt,
  ...
  private async remoteHealth(id: string): Promise<MachineHealth | undefined> {
    ...
    const checkedAt = this.now().toISOString();
    try {
      const response = await this.clientFor(machine).requestJson("GET", "/api/pi-webui/status", undefined, { timeoutMs: DEFAULT_REMOTE_HEALTH_TIMEOUT_MS });
      if (response.statusCode >= 200 && response.statusCode < 300 && isPiWebUiStatusResponse(response.body)) {
  ...
  private async localRuntime(): Promise<MachineRuntime> {
    const checkedAt = this.now().toISOString();
    try {
      return machineRuntime("local", checkedAt, await (this.deps.localRuntime ?? getPiWebUiRuntime)());
    } catch (error) {
  ...
  private async remoteRuntime(id: string): Promise<MachineRuntime | undefined> {
    ...
    const checkedAt = this.now().toISOString();
    try {
      const response = await this.clientFor(machine).requestJson("GET", "/api/pi-webui/runtime", undefined, { timeoutMs: DEFAULT_REMOTE_HEALTH_TIMEOUT_MS });
      const runtime = parsePiWebUiRuntimeResponse(response.body);
...
function componentStatusFromRuntime(runtime: PiWebUiRuntimeComponent): PiWebUiComponentStatus {
  return {
...
function machineRuntime(machineId: string, checkedAt: string, runtime: PiWebUiRuntimeResponse): MachineRuntime {
  return {
    machineId,
    ok: true,
    checkedAt,
...
function isPiWebUiStatusResponse(value: unknown): value is PiWebUiStatusResponse {
  if (!isRecord(value)) return false;
  const components = value["components"];
  if (!isRecord(components)) return false;
  return isPiWebUiComponentStatus(components["web"]) && isPiWebUiComponentStatus(components["sessiond"]);
}

function isPiWebUiComponentStatus(value: unknown): value is PiWebUiComponentStatus {
  if (!isRecord(value)) return false;
