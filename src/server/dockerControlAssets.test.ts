...
const dockerEntrypoint = join(repoRoot, "docker", "pi-webui-docker");
...
beforeEach(async () => {
  tempDir = await mkdtemp(join(tmpdir(), "pi-webui-docker-test-"));
});
...
describe("Docker command assets", () => {
  ...
  it("packages the canonical Docker command and internal support assets", async () => {
    ...
    const customImageHooksIndex = devDockerfile.indexOf("for script in /tmp/pi-webui-custom-image.d/*.sh");
    const dependencyGenerationIndex = devDockerfile.indexOf("/opt/pi-webui-dev-dependencies/generation");

    expect(dockerfile).toContain("COPY pi-webui-docker /usr/local/bin/pi-webui-docker");
    ...
    expect(devDockerfile).toContain("COPY docker/pi-webui-docker /usr/local/bin/pi-webui-docker");
    expect(devDockerfile).toContain("COPY docker/internal/bin/hostexec /usr/local/bin/hostexec");
    expect(devDockerfile).toContain("COPY --chmod=0755 docker/internal/dev/sync-node-modules /usr/local/sbin/pi-webui-dev-sync-node-modules");
    expect(devDockerfile).toContain("/opt/pi-webui-dev-dependencies/node_modules");
    ...
    expect(dependencySync).toContain(".pi-webui-dev-dependency-generation");
    expect(dockerignore).toContain("!pi-webui-docker");
    ...
    expect(devWrapper).toContain("PI_WEBUI_DOCKER_DEV_REPO_ROOT=$repo_root");
    ...
    expect(devCompose).toContain("PI_WEBUI_DOCKER_MODE: dev");
    expect(devCompose).toContain("PI_WEBUI_DOCKER_DEV_REPO_ROOT: ${PI_WEBUI_DOCKER_DEV_REPO_ROOT:?set by docker/pi-webui-docker --dev}");
    expect(devCompose).toContain("PI_WEBUI_DOCKER_HELPER_IMAGE: ${PI_WEBUI_DEV_IMAGE:-pi-webui:dev}");
    expect(devCompose).toContain("COMPOSE_PROJECT_NAME: ${COMPOSE_PROJECT_NAME:-pi-webui-dev}");
    expect(devCompose).toContain("/usr/local/sbin/pi-webui-dev-sync-node-modules");
    expect(devCompose.match(/volumes: \*pi-webui-dev-volumes/g)).toHaveLength(3);
  });
  ...
  dockerCommandIt("fetches remote installer assets without clobbering the write target", async () => {
    ...
    await withUnixSocket(socketPath, async () => {
      await execUtf8("sh", [
        ...
        PI_WEBUI_DOCKER_ASSET_BASE: "https://assets.example.test/docker",
      });
    });

    expect(await readFile(join(installDir, "Dockerfile"), "utf8")).toContain("COPY pi-webui-docker /usr/local/bin/pi-webui-docker");
    expect(await readFile(join(installDir, "pi-webui-docker"), "utf8")).toContain("Usage: pi-webui-docker");
  ...
  dockerCommandIt("runs status through Docker Compose in the foreground", async () => {
    ...
    expect(result.stdout).toContain("fake docker compose --project-name pi-webui-test --env-file .env -f compose.yml -f compose.override.yml ps");
    const log = await readFile(fakeDocker.logPath, "utf8");
    expect(log).toContain("compose version");
    expect(log).toContain("compose --project-name pi-webui-test --env-file .env -f compose.yml -f compose.override.yml ps");
  ...
  dockerCommandIt("runs production host lifecycle commands through the generated runtime env", async () => {
    ...
    const log = await readFile(fakeDocker.logPath, "utf8");
    expect(log).toContain("compose --project-name pi-webui-test --env-file .env -f compose.yml -f compose.override.yml up -d");
    expect(log).toContain("compose --project-name pi-webui-test --env-file .env -f compose.yml -f compose.override.yml down");
    expect(log).toContain("compose --project-name pi-webui-test --env-file .env -f compose.yml -f compose.override.yml restart sessiond");
    expect(log).toContain("compose --project-name pi-webui-test --env-file .env -f compose.yml -f compose.override.yml logs -f web");
    expect(log).toContain("compose --project-name pi-webui-test --env-file .env -f compose.yml -f compose.override.yml exec sessiond bash");
    expect(log).toContain("compose --project-name pi-webui-test --env-file .env -f compose.yml -f compose.override.yml exec web pi-webui config show");
    expect(log).not.toContain("run -d");
  });
  ...
  dockerCommandIt("runs development commands through generated env while preserving host user ids", async () => {
    ...
    await writeFile(runtimeEnvFile, [
      "PI_WEBUI_UID=0",
      "PI_WEBUI_GID=0",
      `PI_WEBUI_DOCKER_DATA_DIR=${runtimeDataDir}`,
      "PI_WEBUI_BIND_ADDR=0.0.0.0",
      "COMPOSE_PROJECT_NAME=runtime-project",
      "",
    ].join("\n"));

    await withUnixSocket(socketPath, async () => {
      await runDockerCommand(["--dev", "status"], devHostEnv(fakeDocker, devRoot, home, { PI_WEBUI_DOCKER_RUNTIME_ENV_FILE: runtimeEnvFile }));
    });

    const generatedEnvPath = join(devRoot, ".pi-webui", "docker-compose-dev.generated.env");
    const generatedEnv = await readFile(generatedEnvPath, "utf8");
    expect(generatedEnv).toContain("PI_WEBUI_UID=1234\n");
    expect(generatedEnv).toContain("PI_WEBUI_GID=2345\n");
    expect(generatedEnv).toContain("DOCKER_GID=0\n");
    expect(generatedEnv).toContain(`PI_WEBUI_DOCKER_DATA_DIR=${runtimeDataDir}\n`);
    expect(generatedEnv).toContain(`PI_WEBUI_DOCKER_DEV_REPO_ROOT=${devRoot}\n`);
    expect(generatedEnv).toContain("COMPOSE_PROJECT_NAME=pi-webui-dev\n");
    expect(generatedEnv).toContain("PI_WEBUI_DEV_API_BIND_ADDR=0.0.0.0\n");
    ...
    await withUnixSocket(socketPath, async () => {
      await runDockerCommand(["--dev", "status"], devHostEnv(fakeDocker, devRoot, home, {
        ...
        PI_WEBUI_DOCKER_RUNTIME_ENV_FILE: runtimeEnvFile,
      }));
    });
    const regeneratedEnv = await readFile(generatedEnvPath, "utf8");
    expect(regeneratedEnv).toContain("COMPOSE_PROJECT_NAME=pi-webui-dev\n");
    expect(regeneratedEnv).toContain(`PI_WEBUI_DOCKER_DATA_DIR=${runtimeDataDir}\n`);
    expect(regeneratedEnv).not.toContain("COMPOSE_PROJECT_NAME=ambient-dev-project");

    const localConfig = await readFile(join(devRoot, ".pi-webui", "docker-compose-dev.local.env"), "utf8");
    expect(localConfig).toContain("docker/pi-webui-docker --dev creates this file once");
    expect(localConfig).toContain("PI_WEBUI_UID and PI_WEBUI_GID default to the current host user");
    const override = await readFile(join(devRoot, ".pi-webui", "docker-compose-dev.host.generated.yml"), "utf8");
    expect(override).toContain(socketPath);
    expect(override).toContain(devRoot);
    const log = await readFile(fakeDocker.logPath, "utf8");
    expect(log).toContain(`compose --project-name pi-webui-dev --env-file ${generatedEnvPath} -f ${devRoot}/docker/compose.dev.yml -f ${devRoot}/.pi-webui/docker-compose-dev.host.generated.yml ps`);
  });
  ...
  dockerCommandIt("passes the root override through to the development compose helper", async () => {
    ...
    await runDockerCommand(["--dev", "--allow-root", "status"], {
      ...cleanProcessEnv(),
      PATH: `${fakeDocker.binDir}:${process.env["PATH"] ?? ""}`,
      PI_WEBUI_DOCKER_DEV_REPO_ROOT: devRoot,
    });
  ...
  dockerCommandIt("refuses dirty development updates before scheduling a detached helper", async () => {
    ...
    expect(result.stdout).not.toContain("Started detached PI WEBUI Docker helper");
  ...
  dockerCommandIt("starts development detached helpers as the generated dev user", async () => {
    ...
    const log = await readFile(fakeDocker.logPath, "utf8");
    expect(log).toContain(`--env-file ${devRoot}/.pi-webui/docker-compose-dev.generated.env`);
    expect(log).toContain("--group-add 3456");
    expect(log).toContain("--user 1234:2345");
    expect(log).toContain("PI_WEBUI_DOCKER_MODE=dev");
    expect(log).toContain("PI_WEBUI_DOCKER_ALLOW_ROOT=0");
    expect(log).toContain("PI_WEBUI_DOCKER_HELPER_IMAGE=pi-webui:test");
    expect(log).toContain(`PI_WEBUI_DOCKER_DEV_REPO_ROOT=${devRoot}`);
    expect(log).toContain("COMPOSE_PROJECT_NAME=pi-webui-dev-test");
    expect(log).toContain("pi-webui.docker-helper.mode=dev");
    expect(log).toContain("pi-webui:test pi-webui-docker --dev __run-detached restart-sessiond");
  ...
  dockerCommandIt("rejects inside-container commands whose explicit mode does not match the container mode", async () => {
    ...
    expect(result.exitCode).not.toBe(0);
    expect(result.stderr).toContain("this PI WEBUI Docker container is in dev mode");
  });
  ...
  dockerCommandIt("explains source checkout runtime-mode mistakes", async () => {
    ...
    expect(result.stderr).toContain("./docker/pi-webui-docker --dev start");
    expect(result.stderr).toContain("/home/pi-webui-test/.local/share/pi-webui-docker/pi-webui-docker start");
    expect(result.stderr).toContain("PI_WEBUI_DOCKER_INSTALL_DIR");
  });
  ...
  dockerCommandIt("starts restart-sessiond in a detached Docker helper", async () => {
    ...
    expect(result.stdout).toContain("Started detached PI WEBUI Docker helper");
    expect(result.stdout).toContain("Streaming detached PI WEBUI Docker helper logs inline.");
    expect(result.stdout).toContain("Reconnect with: docker logs -f pi-webui-docker-restart-sessiond-");
    ...
    expect(log).toContain("--group-add 3456");
    expect(log).toContain("--user 1234:2345");
    expect(log).toContain(`PI_WEBUI_DOCKER_INSTALL_DIR=${installDir}`);
    expect(log).toContain(`PI_WEBUI_DOCKER_DATA_DIR=${join(installDir, "data")}`);
    expect(log).toContain("PI_WEBUI_PORT=12345");
    expect(log).toContain("PI_WEBUI_DOCKER_EXTRA_HOST_PATHS=/srv/pi-webui-extra /opt/pi-webui-extra");
    expect(log).toContain("PI_WEBUI_EXTRA_ZYPPER_PACKAGES=git-lfs jq");
    expect(log).not.toContain('PI_WEBUI_EXTRA_ZYPPER_PACKAGES="git-lfs jq"');
    expect(log).toContain("PI_WEBUI_DOCKER_HELPER_IMAGE=pi-webui:test");
    expect(log).toContain("COMPOSE_PROJECT_NAME=pi-webui-test");
    expect(log).toContain("pi-webui.docker-helper.mode=runtime");
    expect(log).toContain("pi-webui.docker-helper.root=");
    expect(log).toContain("pi-webui.docker-helper.project=pi-webui-test");
    expect(log).toContain("pi-webui:test pi-webui-docker __run-detached restart-sessiond");
    expect(log).not.toContain("--user 0:0");
  ...
  dockerCommandIt("executes the detached runtime update action through Compose without nesting helpers", async () => {
    ...
    const log = await readFile(fakeDocker.logPath, "utf8");
    expect(log).toContain("compose --project-name pi-webui-test --env-file .env -f compose.yml -f compose.override.yml build --pull --no-cache");
    expect(log).toContain("compose --project-name pi-webui-test --env-file .env -f compose.yml -f compose.override.yml up -d --force-recreate --remove-orphans");
    expect(log).not.toContain("run -d");
  });
});
...
async function createRuntimeInstall(): Promise<string> {
  ...
  await writeFile(join(installDir, ".env"), [
    ...
    "PI_WEBUI_GID=2345",
    "DOCKER_GID=3456",
    `PI_WEBUI_DOCKER_DATA_DIR=${join(installDir, "data")}`,
    `PI_WEBUI_DOCKER_INSTALL_DIR=${installDir}`,
    "PI_WEBUI_DOCKER_EXTRA_HOST_PATHS=\"/srv/pi-webui-extra /opt/pi-webui-extra\"",
    "PI_WEBUI_BIND_ADDR=127.0.0.1",
    "PI_WEBUI_PORT=12345",
    "PI_WEBUI_EXTRA_ZYPPER_PACKAGES=\"git-lfs jq\"",
    "PI_WEBUI_IMAGE=pi-webui:test",
    "COMPOSE_PROJECT_NAME=pi-webui-test",
    "",
  ].join("\n"), "utf8");
  await writeFile(join(installDir, "compose.yml"), "name: pi-webui\nservices: {}\n", "utf8");
...
async function createDevRepoFixture(): Promise<string> {
  ...
  await writeFile(join(devRoot, "docker", "compose.dev.yml"), "name: pi-webui-dev\nservices: {}\n", "utf8");
  return devRoot;
}
...
set -eu
printf 'allow=%s args=%s\n' "\${PI_WEBUI_DOCKER_ALLOW_ROOT:-}" "$*" >>${shellSingleQuote(logPath)}
`, "utf8");
...
async function createCleanDevGitRepoWithFakeHelper(logPath: string): Promise<string> {
  ...
  await execUtf8("git", [
    "-C", devRoot,
    "-c", "user.name=PI WEBUI Test",
    "-c", "user.email=pi-webui-test@example.invalid",
...
async function createDevGeneratedEnv(ids: { uid: number; gid: number; dockerGid: number }): Promise<string> {
  ...
  await mkdir(join(devRoot, ".pi-webui"), { recursive: true });
  await writeFile(join(devRoot, ".pi-webui", "docker-compose-dev.generated.env"), [
    `PI_WEBUI_UID=${String(ids.uid)}`,
    `PI_WEBUI_GID=${String(ids.gid)}`,
    `DOCKER_GID=${String(ids.dockerGid)}`,
    `PI_WEBUI_DOCKER_DATA_DIR=${join(tempDir, "dev-data")}`,
    `PI_WEBUI_DOCKER_DEV_REPO_ROOT=${devRoot}`,
    "PI_WEBUI_DEV_API_BIND_ADDR=127.0.0.1",
    "PI_WEBUI_DEV_BIND_ADDR=127.0.0.1",
    "PI_WEBUI_DEV_API_PORT=8808",
    "PI_WEBUI_DEV_PORT=8809",
    "PI_WEBUI_DEV_IMAGE=pi-webui:test",
    "COMPOSE_PROJECT_NAME=pi-webui-dev-test",
    "",
  ].join("\n"), "utf8");
  return devRoot;
}
...
function cleanProcessEnv(): NodeJS.ProcessEnv {
  ...
  for (const key of Object.keys(env)) {
    if (key === "COMPOSE_PROJECT_NAME" || key === "DOCKER_GID" || key === "HOSTEXEC_IMAGE" || key === "XDG_DATA_HOME" || key.startsWith("PI_WEBUI_")) {
...
function devRuntimeEnv(fakeDocker: FakeDocker, devRoot: string): NodeJS.ProcessEnv {
  return {
    ...
    PI_WEBUI_DOCKER_MODE: "dev",
    PI_WEBUI_DOCKER_DEV_REPO_ROOT: devRoot,
    PI_WEBUI_DOCKER_CONTAINER_ID: "current-container",
  };
}
