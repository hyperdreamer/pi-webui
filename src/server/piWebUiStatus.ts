...
import type { ActiveAgentProfileDescriptor, PiWebUiCapability, PiWebUiComponentStatus, PiWebUiInstallationInfo, PiWebUiReleaseStatus, PiWebUiRuntimeComponent, PiWebUiRuntimeResponse, PiWebUiServiceComponent, PiWebUiStatusMessage, PiWebUiStatusResponse, PiWebUiVersionResponse } from "../shared/apiTypes.js";
import { effectivePiWebUiCapabilities, WEB_RUNTIME_CAPABILITIES } from "../shared/capabilities.js";
import { piWebUiDockerCommand } from "../docker/piWebUiDockerCommandPlan.js";
import { parsePiWebUiComponentStatus, parsePiWebUiRuntimeComponent } from "../shared/piWebUiStatusParsing.js";
...
import { createPiWebUiReleaseLookupCache, type PiWebUiReleaseLookup } from "./piWebUiReleaseLookupCache.js";

const PI_WEBUI_PACKAGE_NAME = "@hyperdreamer/pi-webui";
const PI_WEBUI_NPM_SOURCE = `npm:${PI_WEBUI_PACKAGE_NAME}`;
...
const serviceRefs: Record<ServiceId, NativeServiceRef> = {
  sessiond: {
    id: "sessiond",
    systemdName: "pi-webui-sessiond.service",
    launchdLabel: "com.pi-webui.sessiond",
    launchdPlistName: "com.pi-webui.sessiond.plist",
  },
  web: {
    id: "web",
    systemdName: "pi-webui.service",
    launchdLabel: "com.pi-webui.web",
    launchdPlistName: "com.pi-webui.web.plist",
  },
  uiDev: {
    id: "uiDev",
    systemdName: "pi-webui-ui-dev.service",
    launchdLabel: "com.pi-webui.ui-dev",
    launchdPlistName: "com.pi-webui.ui-dev.plist",
  },
};

const startServiceOrder: ServiceId[] = ["sessiond", "web", "uiDev"];
// Restart web/UI before sessiond: when the restart command runs in a pi-webui
...
interface PiWebUiStatusDaemon {
  request(method: string, path: string, body?: unknown): Promise<{ statusCode: number; headers: Record<string, string>; body: string }>;
}

export interface PiWebUiStatusOptions {
  forceReleaseCheck?: boolean;
  activeAgentProfile?: ActiveAgentProfileDescriptor;
  hasCommand?: (command: string) => Promise<boolean>;
}

const latestReleaseLookupCache = createPiWebUiReleaseLookupCache(fetchLatestNpmVersion);
const runtimePackageInfo = readPackageInfoSync();

export function getPiWebUiRuntimeComponent(component: PiWebUiServiceComponent, capabilities: readonly PiWebUiCapability[] = []): PiWebUiRuntimeComponent {
  return {
    component,
...
export async function getPiWebUiRuntime(daemon: PiWebUiStatusDaemon = new SessionDaemonClient()): Promise<PiWebUiRuntimeResponse> {
  const web = getPiWebUiRuntimeComponent("web", WEB_RUNTIME_CAPABILITIES);
  const sessiond = await getSessiondRuntimeComponent(daemon);
  return {
    packageName: PI_WEBUI_PACKAGE_NAME,
    generatedAt: new Date().toISOString(),
    components: { web, sessiond },
    capabilities: effectivePiWebUiCapabilities({ web, sessiond }),
  };
}

export async function getPiWebUiComponentStatus(component: PiWebUiServiceComponent, options: PiWebUiStatusOptions = {}): Promise<PiWebUiComponentStatus> {
  const [installed, installation] = await Promise.all([
    readInstalledPackageInfo(),
    detectPiWebUiInstallation(options.activeAgentProfile?.dir),
  ]);
...
export async function getPiWebUiVersionStatus(daemon: PiWebUiStatusDaemon = new SessionDaemonClient(), options: PiWebUiStatusOptions = {}): Promise<PiWebUiVersionResponse> {
  const [web, sessiond] = await Promise.all([
    getPiWebUiComponentStatus("web", options),
...
export async function getPiWebUiStatus(daemon: PiWebUiStatusDaemon = new SessionDaemonClient(), options: PiWebUiStatusOptions = {}): Promise<PiWebUiStatusResponse> {
  const versionStatus = await getPiWebUiVersionStatus(daemon, options);
...
async function detectPiWebUiInstallation(agentDir?: string): Promise<PiWebUiInstallationInfo> {
...
function detectDockerInstallation(): PiWebUiInstallationInfo | undefined {
  if (!isTruthyEnv("PI_WEBUI_DOCKER_RUNTIME")) return undefined;
  const dockerMode = dockerModeFromEnv(process.env["PI_WEBUI_DOCKER_MODE"]) ?? inferredDockerModeFromRoots();
...
function dockerModeFromEnv(value: string | undefined): PiWebUiInstallationInfo["dockerMode"] | undefined {
  return value === "runtime" || value === "dev" ? value : undefined;
}

function inferredDockerModeFromRoots(): PiWebUiInstallationInfo["dockerMode"] | undefined {
  if (firstNonEmptyEnv("PI_WEBUI_DOCKER_DEV_REPO_ROOT") !== undefined) return "dev";
  if (firstNonEmptyEnv("PI_WEBUI_DOCKER_INSTALL_DIR") !== undefined) return "runtime";
  return undefined;
}

function dockerRootPathFromEnv(mode: PiWebUiInstallationInfo["dockerMode"] | undefined): string | undefined {
  if (mode === "dev") return firstNonEmptyEnv("PI_WEBUI_DOCKER_DEV_REPO_ROOT", "PI_WEBUI_DOCKER_INSTALL_DIR");
  if (mode === "runtime") return firstNonEmptyEnv("PI_WEBUI_DOCKER_INSTALL_DIR", "PI_WEBUI_DOCKER_DEV_REPO_ROOT");
  return firstNonEmptyEnv("PI_WEBUI_DOCKER_INSTALL_DIR", "PI_WEBUI_DOCKER_DEV_REPO_ROOT");
}
...
async function detectPiPackageInstallation(realRoot: string, displayPath: string, agentDir: string): Promise<PiWebUiInstallationInfo | undefined> {
  try {
...
async function detectNpmGlobalInstallation(realRoot: string, displayPath: string): Promise<PiWebUiInstallationInfo | undefined> {
...
async function getSessiondRuntimeComponent(daemon: PiWebUiStatusDaemon): Promise<PiWebUiRuntimeComponent> {
  try {
    ...
    const runtime = parsePiWebUiRuntimeComponent(parsed);
    if (runtime !== undefined) return runtime;
    const legacyVersion = isRecord(parsed) ? parsePiWebUiComponentStatus(parsed["version"]) : undefined;
...
async function getSessiondComponentStatus(daemon: PiWebUiStatusDaemon, options: PiWebUiStatusOptions = {}): Promise<PiWebUiComponentStatus> {
  try {
    const upstream = await daemon.request("GET", "/runtime");
    if (upstream.statusCode < 200 || upstream.statusCode >= 300) {
    ...
    const status = await getPiWebUiComponentStatus("sessiond", options);
...
async function legacySessiondRuntimeComponent(daemon: PiWebUiStatusDaemon): Promise<PiWebUiRuntimeComponent | undefined> {
...
async function legacySessiondComponentStatus(daemon: PiWebUiStatusDaemon): Promise<PiWebUiComponentStatus | undefined> {
  try {
    const upstream = await daemon.request("GET", "/health");
    if (upstream.statusCode < 200 || upstream.statusCode >= 300) return undefined;
    const parsed: unknown = upstream.body === "" ? undefined : JSON.parse(upstream.body);
    return isRecord(parsed) ? parsePiWebUiComponentStatus(parsed["version"]) : undefined;
  } catch {
    return undefined;
  }
}

function runtimeComponentFromStatus(status: PiWebUiComponentStatus): PiWebUiRuntimeComponent {
  return {
...
function unavailableSessiondRuntime(error: string): PiWebUiRuntimeComponent {
  return {
    component: "sessiond",
...
function unavailableSessiond(error: string): PiWebUiComponentStatus {
  return {
    component: "sessiond",
    label: "Session daemon",
    stale: false,
...
async function getLatestReleaseStatus(currentVersion: string, force: boolean): Promise<PiWebUiReleaseStatus> {
  const checkedAtMs = Date.now();
  if (skipVersionCheck()) {
    return { packageName: PI_WEBUI_PACKAGE_NAME, updateAvailable: false, checkedAt: new Date(checkedAtMs).toISOString(), skipped: true };
  }

  return releaseStatusFromCache(await latestReleaseLookupCache.get(currentVersion, { force }), currentVersion);
}

function releaseStatusFromCache(cache: PiWebUiReleaseLookup, currentVersion: string): PiWebUiReleaseStatus {
  return {
    packageName: PI_WEBUI_PACKAGE_NAME,
...
async function fetchLatestNpmVersion(currentVersion: string): Promise<string> {
  const response = await fetch(`https://registry.npmjs.org/${encodeURIComponent(PI_WEBUI_PACKAGE_NAME)}/latest`, {
    headers: {
      accept: "application/json",
      "user-agent": `${PI_WEBUI_PACKAGE_NAME}/${currentVersion}`,
    },
...
async function commandsFor(components: PiWebUiStatusResponse["components"], options: {
  activeAgentProfile: ActiveAgentProfileDescriptor | undefined;
  hasCommand: (command: string) => Promise<boolean>;
}): Promise<PiWebUiStatusResponse["commands"]> {
  ...
  const [serviceCommands, cliCommands] = await Promise.all([
    ...
    piWebUiCliCommands(installation),
  ]);
...
function preferredInstallation(components: PiWebUiStatusResponse["components"]): PiWebUiInstallationInfo | undefined {
...
function dockerCommands(installation: PiWebUiInstallationInfo): PiWebUiStatusResponse["commands"] {
  return {
    update: piWebUiDockerCommand(installation.dockerMode, "update"),
    restart: piWebUiDockerCommand(installation.dockerMode, "restart"),
    restartWeb: piWebUiDockerCommand(installation.dockerMode, "restart-web"),
    restartSessiond: piWebUiDockerCommand(installation.dockerMode, "restart-sessiond"),
    status: piWebUiDockerCommand(installation.dockerMode, "status"),
  };
}

async function piWebUiCliCommands(installation: PiWebUiInstallationInfo | undefined): Promise<NativeServiceCommands> {
  if (installation?.kind !== "npm-global" || !(await hasCommand("pi-webui"))) return {};
  return { restart: "pi-webui restart", status: "pi-webui status" };
}

function restartCommandFor(installation: PiWebUiInstallationInfo | undefined, serviceCommands: NativeServiceCommands, cliCommands: NativeServiceCommands): string | undefined {
...
export async function updateCommandFor(installation: PiWebUiInstallationInfo | undefined, restartCommand: string | undefined, options: {
  activeAgentProfile: ActiveAgentProfileDescriptor | undefined;
  hasCommand: (command: string) => Promise<boolean>;
  ...
  if (installation?.kind === "pi-package") {
    ...
    return `${PI_CODING_AGENT_DIR_ENV}=${shellQuote(profile.dir)} ${shellQuote(profile.command)} update ${shellQuote(installation.source ?? PI_WEBUI_NPM_SOURCE)} && ${restartCommand}`;
  }
  ...
  return `npm install -g ${PI_WEBUI_PACKAGE_NAME} --allow-scripts=node-pty && ${restartCommand}`;
}
...
async function nativeServiceCommands(): Promise<NativeServiceCommands> {
  ...
  return {
    ...(restartable.length === 0 ? {} : { restart: restartNativeServicesCommand(backend, restartable, "pi-webui-restart") }),
    ...(web.length === 0 ? {} : { restartWeb: restartNativeServicesCommand(backend, web, "pi-webui-restart-web") }),
    ...(sessiond.length === 0 ? {} : { restartSessiond: restartNativeServicesCommand(backend, sessiond, "pi-webui-restart-sessiond") }),
...
function restartNativeServicesCommand(backend: NativeServiceBackendKind, refs: NativeServiceRef[], systemdUnit: string): string {
  ...
  // restarting the session daemon kills the pi-webui terminal that launched it.
...
function buildMessages(components: PiWebUiStatusResponse["components"], release: PiWebUiReleaseStatus, commands: PiWebUiStatusResponse["commands"]): PiWebUiStatusMessage[] {
  const messages: PiWebUiStatusMessage[] = [];
  ...
  if (release.updateAvailable && release.latestVersion !== undefined) {
    messages.push({
      id: "update-available",
      severity: "info",
      title: "PI WEBUI update available",
      body: commands.update === undefined
        ? `PI WEBUI ${release.latestVersion} is available${installedVersion === undefined ? "" : `; installed version is ${installedVersion}`}. Update PI WEBUI, then restart the services or processes for this installation.`
        : `PI WEBUI ${release.latestVersion} is available${installedVersion === undefined ? "" : `; installed version is ${installedVersion}`}. Run the update command to update PI WEBUI and restart its services.`,
  ...
  if (!components.sessiond.available) {
    messages.push({
      ...
      body: commands.status === undefined
        ? `PI WEBUI could not check the session daemon version${components.sessiond.error === undefined ? "." : `: ${components.sessiond.error}`}. Check the session daemon service or process that runs this installation.`
        : `PI WEBUI could not check the session daemon version${components.sessiond.error === undefined ? "." : `: ${components.sessiond.error}`}`,
...
function optionalMessageCommand(command: string | undefined): Pick<PiWebUiStatusMessage, "command"> | object {
  return command === undefined ? {} : { command };
}

function skipVersionCheck(): boolean {
  return ["PI_WEBUI_SKIP_VERSION_CHECK", "PI_WEBUI_OFFLINE", "PI_SKIP_VERSION_CHECK", "PI_OFFLINE"].some((key) => {
    const value = process.env[key];
