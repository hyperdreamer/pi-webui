...
import type { ActiveAgentProfileDescriptor, PiWebUiConfigResponse, PiWebUiPluginInfo } from "../shared/apiTypes.js";
...
beforeEach(async () => {
  tempDir = await mkdtemp(join(tmpdir(), "pi-webui-active-profile-app-"));
});
...
describe("buildApp active profile composition", () => {
  ...
  it.each(["unavailable", "invalid"] as const)("returns 503 instead of falling back when the active profile is %s", async (status) => {
    ...
    try {
      ...
      const manifest = await app.inject({ method: "GET", url: "/pi-webui-plugins/manifest.json" });
...
async function writePackagePlugin(root: string, pluginId: string): Promise<void> {
  ...
  await writeFile(join(root, "package.json"), `${JSON.stringify({
    name: `@test/${pluginId}`,
    version: "1.0.0",
    piWebUi: { plugins: [{ id: pluginId, module: "pi-webui-plugin.js" }] },
  }, null, 2)}\n`, "utf8");
  await writeFile(join(root, "pi-webui-plugin.js"), "export default {};\n", "utf8");
}

function emptyConfigService(): PiWebUiConfigService {
  const response: PiWebUiConfigResponse = {
...
function pluginIds(value: unknown): PiWebUiPluginInfo["id"][] {
