...
export const NODE_PTY_GLOBAL_REINSTALL_COMMAND = "npm install -g @hyperdreamer/pi-webui --allow-scripts=node-pty";
...
export function formatNodePtyNativeModuleCheck(check: NodePtyNativeModuleCheck): FormattedNodePtyNativeModuleCheck {
  ...
  return {
    ...
    lines: [
      ...
      "  For a global npm installation, reinstall PI WEBUI with:",
      `    ${NODE_PTY_GLOBAL_REINSTALL_COMMAND}`,
      "  Then run `pi-webui doctor` again.",
    ],
  };
}
