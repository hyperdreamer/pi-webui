...
beforeEach(async () => {
  tempDir = await mkdtemp(join(tmpdir(), "pi-webui-docker-dependencies-test-"));
});
...
describe("Docker development dependency synchronization", () => {
  dockerSyncIt("replaces a stale dependency tree once per image generation", async () => {
    ...
    expect(first.stderr).toContain("Synchronizing PI WEBUI Docker dev dependencies");
    ...
    expect(await readFile(join(fixture.targetDir, ".pi-webui-dev-dependency-generation"), "utf8")).toBe("image-generation-2\n");
...
async function createSyncFixture(): Promise<SyncFixture> {
  ...
  await Promise.all([
    ...
    writeFile(join(targetDir, ".pi-webui-dev-dependency-generation"), "image-generation-1\n", "utf8"),
  ]);
...
function runSync(fixture: SyncFixture): Promise<{ stdout: string; stderr: string; exitCode: number }> {
  return new Promise((resolvePromise) => {
    execFile("bash", [syncScript], {
      ...
      env: {
        ...process.env,
        PI_WEBUI_DEV_WORKSPACE_DIR: fixture.workspaceDir,
        PI_WEBUI_DEV_DEPENDENCY_SEED_DIR: fixture.seedDir,
      },
    }, (error, stdout, stderr) => {
