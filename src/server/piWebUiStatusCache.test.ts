...
import { createPiWebUiStatusCache, type PiWebUiStatusCacheLoadOptions } from "./piWebUiStatusCache.js";

describe("createPiWebUiStatusCache", () => {
  it("serves cached status while it is fresh", async () => {
    const now = 1_000;
    const load = vi.fn(() => Promise.resolve(status("first")));
    const cache = createPiWebUiStatusCache(load, { ttlMs: 100, now: () => now });
  ...
  it.each(["forced-first", "regular-first"] as const)("does not let an older refresh replace a forced result when %s completes", async (completionOrder) => {
    ...
    const load = vi.fn(({ force }: PiWebUiStatusCacheLoadOptions) => force ? forced.promise : regular.promise);
    const cache = createPiWebUiStatusCache(load);
  ...
  it("makes regular refreshes join a pending forced refresh", async () => {
    const deferred = createDeferred<PiWebUiStatusResponse>();
  ...
  it("retains stale status and reports background refresh errors", async () => {
    ...
    const load = vi.fn()
      ...
      .mockResolvedValueOnce(status("second"));
    const cache = createPiWebUiStatusCache(load, { ttlMs: 100, now: () => now, onError });
  ...
  it("abandons an in-flight load when invalidated", async () => {
    const firstLoad = createDeferred<PiWebUiStatusResponse>();
    const secondLoad = createDeferred<PiWebUiStatusResponse>();
    const load = vi.fn()
