 ...
 */
export function evaluateBrowserNavigation(input: string, policy: BrowserEgressPolicy): BrowserNavigationDecision {
  if (!isValidPolicy(policy)) return blocked("policy-invalid");
 ...
 * is one layer of defense; a browser runtime still needs connection-time
 * interception and a network boundary to defeat rebinding and renderer paths.
 */
export async function evaluateBrowserEgress(input: string, policy: BrowserEgressPolicy, resolver: BrowserDnsResolver): Promise<BrowserNavigationDecision> {
  ...
  try {
    addresses = await resolver.resolve(navigation.host);
  } catch {
    return blockedWithOrigin("dns-unavailable", navigation.auditOrigin);
  }

  if (addresses.length === 0) return blockedWithOrigin("dns-no-addresses", navigation.auditOrigin);
  if (!addresses.every(isPublicInternetAddress)) return blockedWithOrigin("non-public-address", navigation.auditOrigin);
  return navigation;
}
...
function isValidPolicy(policy: BrowserEgressPolicy): boolean {
  if (!BROWSER_EGRESS_MODES.has(policy.mode)) return false;
  if (!Array.isArray(policy.allowedPorts) || !policy.allowedPorts.every((port) => Number.isInteger(port) && port >= 1 && port <= 65_535)) return false;
  if (!Array.isArray(policy.allowedDomains)) return false;
  const normalizedDomains = policy.allowedDomains.map(normalizeAllowedDomain);
  if (normalizedDomains.some((domain) => domain === undefined)) return false;
  if (policy.mode === "domain-allowlist" && normalizedDomains.length === 0) return false;
  return true;
}
