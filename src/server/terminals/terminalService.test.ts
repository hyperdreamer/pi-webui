...
describe.skipIf(process.platform === "win32")("TerminalService command runs", () => {
  ...
  describe("PI_WEBUI_TERMINAL propagation", () => {
    let originalPiWebUiTerminal: string | undefined;

    beforeEach(() => {
      originalPiWebUiTerminal = process.env["PI_WEBUI_TERMINAL"];
      process.env["PI_WEBUI_TERMINAL"] = "conflicting-parent-value";
    });

    afterEach(() => {
      if (originalPiWebUiTerminal === undefined) {
        delete process.env["PI_WEBUI_TERMINAL"];
      } else {
        process.env["PI_WEBUI_TERMINAL"] = originalPiWebUiTerminal;
      }
    });

    it("sets PI_WEBUI_TERMINAL for terminal commands", async () => {
      const service = new TerminalService();
      try {
        const frame = "__PI_WEBUI_RUN_ENV_7F3A9C__";
        const run = service.runCommand({
          origin: "core",
          projectId: "p1",
          workspaceId: "w1",
...
const LOGIN_PROFILE_COMMAND = "pi-webui-test-login-profile-command";
const LOGIN_PROFILE_OUTPUT = "__PI_WEBUI_LOGIN_PROFILE_PATH_COMMAND__";

async function withBashLoginProfile(run: () => Promise<void>): Promise<void> {
  const home = await mkdtemp(join(tmpdir(), "pi-webui-terminal-home-"));
