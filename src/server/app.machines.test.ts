...
import { appTestContext, configFromMachineConfigWriteBody, fakeRemoteClient, fullPiWebUiConfig, piWebUiConfigResponse, registerAppTestHooks, selectedMachinePiWebUiConfig } from "./app.testSupport.js";
...
describe("buildApp machine routes", () => {
  ...
  it("reports effective machine runtime capabilities for remote machines", async () => {
    ...
    const requestJson = vi.fn<MachineClient["requestJson"]>(() => Promise.resolve({
      ...
      body: {
        ...
        generatedAt: "2026-05-25T00:00:00.000Z",
        components: {
          web: { component: "web", label: "Remote Web", runtimeVersion: "1.0.0", available: true, capabilities: [PI_WEBUI_CAPABILITIES.sessionsDeleteArchived, PI_WEBUI_CAPABILITIES.piPackagesManage, PI_WEBUI_CAPABILITIES.agentProfileConfig, "future.capability"] },
          sessiond: {
            component: "sessiond",
        ...
        capabilities: [PI_WEBUI_CAPABILITIES.sessionsDeleteArchived, PI_WEBUI_CAPABILITIES.piPackagesManage, PI_WEBUI_CAPABILITIES.agentProfileConfig, "future.capability"],
      },
    }));
    appTestContext.remoteClient = fakeRemoteClient({ requestJson });
    ...
    expect(runtime.json()).toMatchObject({
      machineId: remote.id,
      ok: true,
      capabilities: [PI_WEBUI_CAPABILITIES.sessionsDeleteArchived, PI_WEBUI_CAPABILITIES.piPackagesManage, PI_WEBUI_CAPABILITIES.agentProfileConfig],
      components: { sessiond: { activeAgentProfile: { command: "remote-agent", dir: "/srv/remote-agent" } } },
    });
    expect(requestJson).toHaveBeenCalledTimes(2);
    expect(requestJson).toHaveBeenCalledWith("GET", "/api/pi-webui/runtime", undefined, { timeoutMs: 3000 });
  });
  ...
  it("filters remote selected-machine config reads to machine-safe keys", async () => {
    ...
    const requestJson = vi.fn<MachineClient["requestJson"]>(() => Promise.resolve({
      ...
      body: piWebUiConfigResponse(fullPiWebUiConfig()),
    }));
    appTestContext.remoteClient = fakeRemoteClient({ requestJson });
    ...
    expect(response.json<PiWebUiConfigResponse>()).toEqual({
      ...piWebUiConfigResponse(fullPiWebUiConfig()),
  ...
  it("rejects a false-success agent profile write from an older remote machine", async () => {
    const addResponse = await appTestContext.app.inject({ method: "POST", url: "/api/machines", payload: { name: "Remote", baseUrl: "https://remote.example.test/" } });
    const remote = addResponse.json<{ id: string }>();
    const legacyConfig = fullPiWebUiConfig();
    delete legacyConfig.agent;
    const requestJson = vi.fn<MachineClient["requestJson"]>(() => Promise.resolve({
      statusCode: 200,
      headers: { "content-type": "application/json" },
      body: piWebUiConfigResponse(legacyConfig),
    }));
    appTestContext.remoteClient = fakeRemoteClient({ requestJson });
  ...
  it("verifies an explicit remote profile reset instead of treating an empty profile as no patch", async () => {
    ...
    const requestJson = vi.fn<MachineClient["requestJson"]>((method) => {
      const config = fullPiWebUiConfig();
      if (method === "PUT") delete config.agent;
      return Promise.resolve({ statusCode: 200, headers: { "content-type": "application/json" }, body: piWebUiConfigResponse(config) });
    });
    appTestContext.remoteClient = fakeRemoteClient({ requestJson });
  ...
  it("preserves foreign-platform agent paths while the target verifies persistence", async () => {
    ...
    const requestJson = vi.fn<MachineClient["requestJson"]>((method, _path, body) => {
      const config = method === "PUT" ? configFromMachineConfigWriteBody(body) : fullPiWebUiConfig();
      return Promise.resolve({ statusCode: 200, headers: { "content-type": "application/json" }, body: piWebUiConfigResponse(config) });
    });
    appTestContext.remoteClient = fakeRemoteClient({ requestJson });
