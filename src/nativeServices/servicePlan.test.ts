...
function productionInput(): ProductionNativeServicePlanInput {
  return {
    backend,
    shell,
    environment: { PI_WEBUI_CONFIG: "/home/user/.config/pi-webui/config.json" },
    executables: {
      sessiond: {
...
describe("production native service planning", () => {
  it("selects named commands from one authoritative backend probe and carries their exact requirements", async () => {
    ...
    expect(requests).toEqual([
      {
        ...
        prerequisites: [
          expect.objectContaining({ id: "sessiond.command.pi-webui-sessiond", kind: "command-available", command: "pi-webui-sessiond" }),
          expect.objectContaining({ id: "web.command.pi-webui-server", kind: "command-available", command: "pi-webui-server" }),
        ],
      },
    ]);
    ...
    expect(resolution.plan).toMatchObject({
      ...
      services: [
        {
          id: "sessiond",
          manager: { systemdName: "pi-webui-sessiond.service", launchdLabel: "com.pi-webui.sessiond" },
          shellCommand: "exec pi-webui-sessiond",
          strategy: { kind: "named-command", command: "pi-webui-sessiond", selectedBy: "authoritative-backend-probe" },
          environment: { PI_WEBUI_CONFIG: "/home/user/.config/pi-webui/config.json" },
          workingDirectory: null,
          after: [],
          wants: [],
          prerequisites: [
            { id: "sessiond.command.pi-webui-sessiond", kind: "command-available", command: "pi-webui-sessiond" },
            { id: "sessiond.node", kind: "node-version", command: "node", minimumVersion: "22.19.0" },
          ],
        },
        {
          id: "web",
          shellCommand: "exec pi-webui-server",
          strategy: { kind: "named-command", command: "pi-webui-server" },
          after: ["sessiond"],
          wants: ["sessiond"],
          prerequisites: [
            { id: "web.command.pi-webui-server", kind: "command-available", command: "pi-webui-server" },
    ...
    expect(planValidationProbeRequests(resolution.plan)).toMatchObject([
      {
        ...
        workingDirectory: null,
        prerequisites: [
          { id: "sessiond.command.pi-webui-sessiond" },
          { id: "sessiond.node" },
          { id: "web.command.pi-webui-server" },
          { id: "web.node" },
        ],
      },
    ]);
  ...
  it("preserves configured overrides verbatim and never probes or executes them", async () => {
    ...
    input.executables.sessiond.configuredCommand = "  /opt/pi webui/run-sessiond --flag  ";
    ...
    expect(resolution.plan.services).toMatchObject([
      {
        id: "sessiond",
        shellCommand: "exec   /opt/pi webui/run-sessiond --flag  ",
        strategy: {
          kind: "configured-override",
          command: "  /opt/pi webui/run-sessiond --flag  ",
  ...
  it("mixes configured, named, and bundled decisions without unrelated checks", async () => {
    ...
    const resolution = await resolveProductionNativeServicePlan(input, {
      probe: {
        run: (request) => {
          ...
          return Promise.resolve({
            kind: "completed",
            outcomes: [{ prerequisiteId: "web.command.pi-webui-server", status: "unsatisfied", detail: null }],
          });
        },
      },
      fileExists: () => true,
    ...
    expect(requests[0]?.prerequisites).toMatchObject([{ id: "web.command.pi-webui-server", command: "pi-webui-server" }]);
    expect(resolution.ok).toBe(true);
    if (resolution.ok) {
  ...
  it("treats thrown and malformed probe results as infrastructure failures", async () => {
    ...
    const malformed = await resolveProductionNativeServicePlan(productionInput(), {
      probe: {
        run: () => Promise.resolve({
          kind: "completed",
          outcomes: [{ prerequisiteId: "sessiond.command.pi-webui-sessiond", status: "satisfied", detail: null }],
        }),
      },
      fileExists: () => true,
    });
    expect(malformed).toMatchObject({
      ok: false,
      failures: [{ kind: "probe-infrastructure", reason: "malformed-output", message: "Authoritative probe returned no outcome for web.command.pi-webui-server." }],
    });
  });
});
...
describe("development native service planning", () => {
  it("plans only the exact checkout commands and prerequisites", () => {
    const plan = createDevelopmentNativeServicePlan({
      ...
      environment: { PI_WEBUI_CONFIG: "/tmp/config.json" },
    ...
    expect(serviceCommandRequirements).not.toContain("pi-webui-server");
    expect(serviceCommandRequirements).not.toContain("pi-webui-sessiond");
