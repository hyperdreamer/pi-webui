...
export class SystemdNativeServiceProbe implements NativeServiceAuthoritativeProbe {
  ...
  public async run(request: NativeServiceProbeRequest): Promise<NativeServiceProbeResult> {
    ...
    const unitName = `pi-webui-authoritative-probe-${uniqueId}.service`;
    const outputPrefix = `PI_WEBUI_PROBE_${uniqueId}`;
...
export class LaunchdNativeServiceProbe implements NativeServiceAuthoritativeProbe {
  ...
  public async run(request: NativeServiceProbeRequest): Promise<NativeServiceProbeResult> {
    ...
    const uniqueId = safeUniqueId(this.dependencies.createUniqueId());
    const label = `com.pi-webui.authoritative-probe.${String(this.dependencies.uid)}.${uniqueId}`;
    ...
    try {
      directory = await this.dependencies.fileSystem.createTemporaryDirectory(
        posixPath.join(tmpdir(), "pi-webui-launchd-probe-"),
      );
...
function externalExecutableShellCheck(
  ...
  if (shell === "fish") {
    const invocation = quotedArguments === "" ? "" : `; and $pi_webui_probe_executable[1] ${quotedArguments}`;
    return `set -l pi_webui_probe_executable (command -v ${quotedCommand}); and test (count $pi_webui_probe_executable) -eq 1; and string match -q '*/*' -- $pi_webui_probe_executable[1]; and test -f $pi_webui_probe_executable[1]; and test -x $pi_webui_probe_executable[1]${invocation}`;
  }
  const invocation = quotedArguments === "" ? "" : ` && "$pi_webui_probe_executable" ${quotedArguments}`;
  return `pi_webui_probe_executable=$(command -v ${quotedCommand}) && case "$pi_webui_probe_executable" in */*) test -f "$pi_webui_probe_executable" && test -x "$pi_webui_probe_executable"${invocation};; *) false;; esac`;
}
